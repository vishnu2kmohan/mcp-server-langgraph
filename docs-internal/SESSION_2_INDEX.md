...
| P0-2: GKE CPU Ratio (OTEL) | `deployments/overlays/stg-gke/otel-collector-patch.yaml` | ⚠️ Partial | CRITICAL |
| P0-3: Network Policy JGroups | `deployments/overlays/stg-gke/network-policy.yaml` | ✅ Fixed | CRITICAL |
| P0-4: Cloud SQL Proxy Probes | `deployments/overlays/stg-gke/cloud-sql-proxy-patch.yaml` | ⚠️ Check | CRITICAL |
...
| P2-2: OTEL Config Validation | `deployments/overlays/stg-gke/otel-collector-configmap-patch.yaml` | ✅ Fixed | MEDIUM |
...
python scripts/validate_gke_autopilot_compliance.py deployments/overlays/stg-gke
...
kustomize build deployments/overlays/stg-gke/ | \
  kubectl apply --dry-run=client -f -
```
...
- `deployments/overlays/stg-gke/keycloak-patch.yaml` - Keycloak hardening
- `deployments/overlays/stg-gke/otel-collector-patch.yaml` - OTEL CPU fix
- `deployments/overlays/stg-gke/network-policy.yaml` - JGroups validation
- `deployments/overlays/stg-gke/cloud-sql-proxy-patch.yaml` - Health probes

**Configuration Files**:
- `deployments/argocd/applications/mcp-server-app.yaml` - Helm values schema (P2-1)
- `deployments/overlays/stg-gke/otel-collector-configmap-patch.yaml` - OTEL config (P2-2)
...
# 3. Fix: Edit deployments/overlays/stg-gke/otel-collector-patch.yaml
...
# 5. Commit:
git add deployments/overlays/stg-gke/otel-collector-patch.yaml
...
Critical Deployment Files:
  ...
  deployments/overlays/stg-gke/keycloak-patch.yaml
  deployments/overlays/stg-gke/otel-collector-patch.yaml
