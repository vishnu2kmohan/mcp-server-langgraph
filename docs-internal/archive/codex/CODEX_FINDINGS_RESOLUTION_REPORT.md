...
**Files Modified**:
- `deployments/overlays/stg-gke/openfga-json-patch.yaml:8`
...
2. **Trigger Staging Deployment**: Verify Kustomize patch fix
   ```bash
   gh workflow run deploy-stg-gke.yaml --ref main
   ```
...
- `deployments/overlays/stg-gke/openfga-json-patch.yaml` (1 fix)
