...
| `GKE_PROD_CLUSTER` | No | `prod-mcp-server-langgraph-gke` | Production GKE cluster name |
...
| `PRODUCTION_NAMESPACE` | No | `prod-mcp-server-langgraph` | Kubernetes namespace for production |
...
| `GCP_PROD_SA_EMAIL` | ✅ Yes | `SERVICE_ACCOUNT@PROJECT_ID.iam.gserviceaccount.com` | Production service account email |
...
GCP_PROD_SA_EMAIL=mcp-prod-sa@vishnu-sandbox-20250310.iam.gserviceaccount.com
```
...
gcloud iam service-accounts create mcp-prod-sa \
...
gcloud projects add-iam-policy-binding YOUR_PROJECT_ID \
    --member="serviceAccount:mcp-prod-sa@YOUR_PROJECT_ID.iam.gserviceaccount.com" \
    --role="roles/container.developer"
...
gcloud iam service-accounts add-iam-policy-binding mcp-prod-sa@YOUR_PROJECT_ID.iam.gserviceaccount.com \
    --role="roles/iam.workloadIdentityUser" \
    --member="principalSet://iam.googleapis.com/projects/PROJECT_NUMBER/locations/global/workloadIdentityPools/github-actions-pool/attribute.repository/YOUR_ORG/YOUR_REPO"
...
- `GCP_STAGING_SA_EMAIL` or `GCP_PROD_SA_EMAIL` (secret)
