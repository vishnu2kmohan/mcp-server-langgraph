...
**Date**: 2025-10-21
**Environment**: GKE Staging (stg-gke)
**Status**: ✅ Complete
...
#### `scripts/gcp/setup-vertex-ai-stg.sh` (NEW)
...
**Usage**:
```bash
./scripts/gcp/setup-vertex-ai-stg.sh
```

### 3. Kubernetes Manifests

#### `deployments/overlays/stg-gke/serviceaccount-patch.yaml`
...
#### `.github/workflows/deploy-stg-gke.yaml`
...
2. Rename environment from `staging` to `stg-gke`
3. Or create new `stg-gke` environment with same protection rules
...
1. **Update GitHub Environment**:
   - Rename `staging` environment to `stg-gke` in GitHub settings
   - Or create new `stg-gke` environment with protection rules
...
- `deployments/overlays/stg-gke/serviceaccount-patch.yaml` - Updated WI annotation
- `deployments/overlays/stg-gke/deployment-patch.yaml` - Added Vertex AI env vars
- `.env.example` - Added Vertex AI configuration examples
- `.github/workflows/deploy-stg-gke.yaml` - Updated environment name

### Created
- `scripts/gcp/setup-vertex-ai-stg.sh` - Automated setup script
...
- **Setup Script**: `scripts/gcp/setup-vertex-ai-stg.sh`
