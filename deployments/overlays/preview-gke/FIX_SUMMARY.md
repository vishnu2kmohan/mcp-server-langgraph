...
- `deployments/overlays/preview-gke/openfga-patch.yaml`
...
**Files Modified**:
- `deployments/overlays/preview-gke/redis-session-service-json-patch.yaml` (new)
- `deployments/overlays/preview-gke/deployment-init-containers-patch.yaml`
- `deployments/overlays/preview-gke/kustomization.yaml`

### 3. Service Name Mismatches (P1)

**Problem**: Init containers referenced unprefixed service names (e.g., `openfga`) but Kustomize applied `preview-` prefix to all services, causing DNS lookup failures.

**Fix**: Updated init container patches to use prefixed names:
- `openfga` → `preview-openfga`
- `keycloak` → `preview-keycloak`
- `redis-session` → `preview-redis-session`
