...
1. **PostgreSQL** → **Cloud SQL**
   - Instance: `mcp-staging-postgres` (10.110.0.3)
...
2. **Redis** → **Memorystore Redis**
   - Instance: `mcp-staging-redis` (10.110.1.4:6379)
...
1. **Cloud SQL Instance**: `mcp-staging-postgres` must exist
2. **Memorystore Redis**: `mcp-staging-redis` must exist
...
```bash
bash scripts/gcp/setup-staging-infrastructure.sh
```
...
gcloud redis instances describe mcp-staging-redis --region=us-central1

# Test connection from pod
kubectl run -it --rm redis-test --image=redis:alpine -n mcp-staging -- \
  redis-cli -h 10.110.1.4 -a $(gcloud secrets versions access latest --secret=staging-redis-password) ping
```
...
- Cloud SQL Reference: `docs/deployment/gcp-staging-infrastructure.md`
