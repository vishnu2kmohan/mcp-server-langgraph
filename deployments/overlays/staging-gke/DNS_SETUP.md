...
```bash
gcloud redis instances describe staging-redis \
  --region=us-central1 \
  --format='get(host)' \
  --project=YOUR_PROJECT_ID
```
...
```bash
gcloud redis instances describe staging-redis-session \
  --region=us-central1 \
  --format='get(host)' \
  --project=YOUR_PROJECT_ID
```
...
```bash
gcloud dns managed-zones describe staging-internal --project=YOUR_PROJECT_ID
```
...
```bash
gcloud dns record-sets list --zone="staging-internal" --project=YOUR_PROJECT_ID
```
