# GKE stg Overlay - Cloud-Native Architecture

**Last Updated**: 2025-11-02
**Purpose**: Production-ready GKE stg environment using cloud-managed services
...
1. **PostgreSQL** → **Cloud SQL**
   - Instance: `mcp-stg-postgres` (10.110.0.3)
...
2. **Redis** → **Memorystore Redis**
   - Instance: `mcp-stg-redis` (10.110.1.4:6379)
...
- name: cloud-sql-proxy
  ...
  args:
    - "vishnu-sandbox-20250310:us-central1:mcp-stg-postgres"
    - "--port=5432"
```
...
The following base resources are **intentionally excluded** for GKE stg:
...
1. **Cloud SQL Instance**: `mcp-stg-postgres` must exist
2. **Memorystore Redis**: `mcp-stg-redis` must exist
...
```bash
bash scripts/gcp/setup-stg-infrastructure.sh
```
...
kubectl get pods -n mcp-stg

# Verify Cloud SQL connectivity
kubectl logs -n mcp-stg -l app=mcp-server-langgraph -c cloud-sql-proxy

# Verify Memorystore Redis connectivity
kubectl exec -n mcp-stg -it POD_NAME -- redis-cli -h 10.110.1.4 -a PASSWORD ping
```
...
kubectl logs -n mcp-stg POD_NAME -c cloud-sql-proxy

# Verify Workload Identity
gcloud iam service-accounts get-iam-policy \
  mcp-stg-sa@vishnu-sandbox-20250310.iam.gserviceaccount.com

# Test connection
kubectl exec -n mcp-stg -it POD_NAME -- nc -zv 127.0.0.1 5432
```
...
gcloud redis instances describe mcp-stg-redis --region=us-central1

# Test connection from pod
kubectl run -it --rm redis-test --image=redis:alpine -n mcp-stg -- \
  redis-cli -h 10.110.1.4 -a $(gcloud secrets versions access latest --secret=stg-redis-password) ping
```
...
- Infrastructure Setup: `scripts/gcp/setup-stg-infrastructure.sh`
- Cloud SQL Reference: `docs/deployment/gcp-stg-infrastructure.md`
