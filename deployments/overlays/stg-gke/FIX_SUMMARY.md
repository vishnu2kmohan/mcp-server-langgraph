# GKE STG Deployment Fixes - 2025-11-06
...
- `deployments/overlays/stg-gke/openfga-patch.yaml`
...
**Files Modified**:
- `deployments/overlays/stg-gke/redis-session-service-json-patch.yaml` (new)
- `deployments/overlays/stg-gke/deployment-init-containers-patch.yaml`
- `deployments/overlays/stg-gke/kustomization.yaml`

### 3. Service Name Mismatches (P1)

**Problem**: Init containers referenced unprefixed service names (e.g., `openfga`) but Kustomize applied `stg-` prefix to all services, causing DNS lookup failures.

**Fix**: Updated init container patches to use prefixed names:
- `openfga` → `stg-openfga`
- `keycloak` → `stg-keycloak`
- `redis-session` → `stg-redis-session`
