# Cloud DNS Setup for stg GKE Overlay

This document explains how to configure Cloud DNS records for the stg environment to enable robust failover support for Cloud SQL and Memorystore Redis.
...
```bash
gcloud sql instances describe stg-postgres \
...
```bash
gcloud redis instances describe stg-redis \
  --region=us-central1 \
  --format='get(host)' \
  --project=YOUR_PROJECT_ID
```
...
**DNS Record:**
```
Name:  redis-session-stg.internal
Type:  A
TTL:   300
...
```bash
gcloud redis instances describe stg-redis-session \
  --region=us-central1 \
  --format='get(host)' \
  --project=YOUR_PROJECT_ID
```
...
```bash
gcloud dns managed-zones create stg-internal \
  --description="Private DNS for stg environment" \
  --dns-name="stg.internal" \
  --networks="default" \
  --visibility="private" \
  --project=YOUR_PROJECT_ID
```
...
```bash
# Cloud SQL
gcloud dns record-sets create cloudsql-stg.internal. \
  --zone="stg-internal" \
  --type="A" \
  --ttl="300" \
...
# Memorystore Redis
gcloud dns record-sets create redis-stg.internal. \
  --zone="stg-internal" \
  --type="A" \
  --ttl="300" \
  --rrdatas="<REDIS_IP>" \
...
# Redis Session Store
gcloud dns record-sets create redis-session-stg.internal. \
  --zone="stg-internal" \
  --type="A" \
  --ttl="300" \
...
```hcl
resource "google_dns_managed_zone" "stg_internal" {
  name        = "stg-internal"
  dns_name    = "stg.internal."
  description = "Private DNS for stg environment"
  visibility  = "private"
...
resource "google_dns_record_set" "cloudsql_stg" {
  name         = "cloudsql-stg.internal."
  managed_zone = google_dns_managed_zone.stg_internal.name
  type         = "A"
  ttl          = 300
  rrdatas      = [google_sql_database_instance.stg_postgres.private_ip_address]
}

resource "google_dns_record_set" "redis_stg" {
  name         = "redis-stg.internal."
  managed_zone = google_dns_managed_zone.stg_internal.name
  type         = "A"
  ttl          = 300
  rrdatas      = [google_redis_instance.stg_redis.host]
}

resource "google_dns_record_set" "redis_session_stg" {
  name         = "redis-session-stg.internal."
  managed_zone = google_dns_managed_zone.stg_internal.name
  type         = "A"
  ttl          = 300
  rrdatas      = [google_redis_instance.stg_redis_session.host]
}
```
...
# Inside the pod:
nslookup cloudsql-stg.internal
nslookup redis-stg.internal
nslookup redis-session-stg.internal
...
```bash
gcloud dns record-sets update cloudsql-stg.internal. \
  --zone="stg-internal" \
  --type="A" \
  --ttl="300" \
  --rrdatas="<NEW_IP>" \
...
```bash
kubectl rollout restart deployment/mcp-server-langgraph -n stg-mcp-server-langgraph
```
...
```bash
gcloud dns managed-zones describe stg-internal --project=YOUR_PROJECT_ID
```
...
```bash
gcloud dns record-sets list --zone="stg-internal" --project=YOUR_PROJECT_ID
```
