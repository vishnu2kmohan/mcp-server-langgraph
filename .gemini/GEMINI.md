...
Gemini Code Assist runs in a Docker-based sandbox with read-only mounts:
- Workspace is mounted at `/workspace:ro`
- Use `read_file` for file access
...
When generating structured output (reviews, analysis):
- Use `--output-format json` for machine-parseable results
- Include file paths, line numbers, and severity in findings
- Follow the project's category/severity definitions
