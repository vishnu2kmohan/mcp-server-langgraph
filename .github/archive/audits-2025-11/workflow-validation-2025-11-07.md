...
**New Pre-Deployment Checks** (`deploy-stg-gke.yaml:49-81`):
```yaml
pre-deployment-checks:
  ...
  steps:
    - name: Validate Kustomize configuration
      run: kubectl kustomize deployments/overlays/stg-gke > /tmp/manifests.yaml
    ...
    - name: Security scan manifests
      ...
      with:
        scan-type: 'config'
        scan-ref: 'deployments/overlays/stg-gke'
...
9. `.github/workflows/deploy-stg-gke.yaml` - hard-coded paths, pre-deployment validation
10. `.github/workflows/deploy-prod-gke.yaml` - docker action versions
