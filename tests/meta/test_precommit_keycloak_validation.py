...
class TestKeycloakConfigValidationHook:
    ...
    def test_hook_detects_keycloak_service_enabled(self):
        ...
        try:
            ...
            result = subprocess.run(
                ["python", "scripts/validation/validate_keycloak_config.py", str(temp_file)], capture_output=True, text=True, timeout=60
            )
    ...
    def test_hook_with_production_docker_compose_file(self):
        ...
        result = subprocess.run(
            ["python", "scripts/validation/validate_keycloak_config.py", str(compose_file)], capture_output=True, text=True, timeout=60
        )
