...
def test_keycloak_realm_import_file_exists(repo_root: Path):
    ...
    assert realm_file.exists(), (
        ...
        "\n"
        "Fix: Create tests/e2e/mcp-test.json with realm configuration"
    )
...
def test_docker_compose_imports_realm(repo_root: Path):
    ...
    for volume in volumes:
        if isinstance(volume, str):
            ...
            if "mcp-test.json" in volume and "/opt/keycloak/data/import" in volume:
                realm_volume_found = True
                break
        elif isinstance(volume, dict):
            # Dict format with source/target
            if volume.get("source") and "mcp-test.json" in str(volume.get("source")):
                realm_volume_found = True
                break
    ...
    assert realm_volume_found, (
        ...
        "  ./tests/e2e/mcp-test.json:/opt/keycloak/data/import/mcp-test.json\n"
        "\n"
        ...
        "  keycloak-test:\n"
        "    volumes:\n"
        "      - ./tests/e2e/mcp-test.json:/opt/keycloak/data/import/mcp-test.json:ro"
    )
