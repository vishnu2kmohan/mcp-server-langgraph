...
def test_trivyignore_exists_for_staging():
    """
    Verify that .trivyignore file exists in the stg-gke overlay directory.
    ...
    trivyignore_file = repo_root / "deployments/overlays/stg-gke/.trivyignore"

    assert trivyignore_file.exists(), (
        f".trivyignore file not found: {trivyignore_file}\n"
        "\n"
        "Expected file: deployments/overlays/stg-gke/.trivyignore\n"
...
def test_trivyignore_workflow_integration():
    """
    Verify that the deploy-stg-gke workflow is configured to use .trivyignore.

    The Trivy action should reference the trivyignores file so suppressions are applied.
    """
    repo_root = Path(__file__).parent.parent.parent
    workflow_file = repo_root / ".github/workflows/deploy-stg-gke.yaml"
