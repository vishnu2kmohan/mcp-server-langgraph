...
class TestKustomizeStagingGKE:
    ...
    def test_secret_references_use_staging_prefix(self):
        """
        ...
        for manifest in manifests:
            ...
            for container in containers + init_containers:
                ...
                for env_name, secret_name in secret_refs:
                    # All secrets in staging-gke should have 'staging-' prefix
                    if not secret_name.startswith("staging-"):
                        violations.append(
                            {
                                "deployment": name,
                                ...
                                "issue": f"Secret reference '{secret_name}' missing 'staging-' prefix",
                            }
                        )

        if violations:
            error_msg = "\n\nSecret reference violations in staging-gke:\n"
            error_msg += "\nKustomize namePrefix='staging-' doesn't auto-update secretKeyRef!\n"
            ...
            for v in violations:
                ...
                error_msg += f"\n    Expected: staging-{v['secret_name']}"
                error_msg += f"\n    Issue: {v['issue']}\n"
    ...
    def test_keycloak_volume_mounts_dont_override_critical_paths(self):
        ...
        keycloak_deployment = self._find_resource(manifests, "Deployment", "staging-keycloak")

        if not keycloak_deployment:
            pytest.skip("staging-keycloak deployment not found in rendered manifests")
        ...
        if violations:
            error_msg = "\n\nCritical path volume mount violations in staging-keycloak:\n"
    ...
    def test_keycloak_has_required_volume_mounts(self):
        ...
        if missing_mounts:
            error_msg = "\n\nMissing required volume mounts in staging-keycloak:\n"
