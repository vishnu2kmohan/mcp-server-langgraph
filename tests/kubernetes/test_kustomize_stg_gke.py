"""
Test suite to validate stg-gke Kustomize configuration correctness.

These tests validate that Kustomize rendered manifests are properly configured
for the stg-gke environment, catching issues before deployment.
...
Issue Context:
- Pod failure: stg-mcp-server-langgraph (CreateContainerConfigError)
  Root cause: Secret name mismatch - deployment refs 'mcp-server-langgraph-secrets'
  but Kustomize creates 'stg-mcp-server-langgraph-secrets' due to namePrefix

- Pod failure: stg-keycloak (CrashLoopBackOff)
...
STG_GKE_OVERLAY = PROJECT_ROOT / "deployments" / "overlays" / "stg-gke"


@pytest.mark.unit
@pytest.mark.kubernetes
@pytest.mark.requires_kubectl
@pytest.mark.xdist_group(name="test_kustomize_stg_gke")
class TestKustomizeStgGKE:
    """Test that stg-gke Kustomize configurations are correct."""
    ...
    def _render_kustomize_manifests(self) -> list[dict[str, Any]]:
        ...
        if not STG_GKE_OVERLAY.exists():
            pytest.skip(f"STG GKE overlay not found: {STG_GKE_OVERLAY}")

        try:
            # Run kubectl kustomize to render manifests
            result = subprocess.run(
                ["kubectl", "kustomize", str(STG_GKE_OVERLAY)],
                capture_output=True,
                text=True,
                check=True,
                timeout=30,
    ...
    def test_secret_references_use_stg_prefix(self):
        """
        Test that all secret references in stg-gke use 'stg-' prefix.
        ...
        for manifest in manifests:
            ...
            for container in containers + init_containers:
                ...
                for env_name, secret_name in secret_refs:
                    # All secrets in stg-gke should have 'stg-' prefix
                    if not secret_name.startswith("stg-"):
                        violations.append(
                            {
                                "deployment": name,
                                ...
                                "expected_prefix": "stg-",
                                "issue": f"Secret reference '{secret_name}' missing 'stg-' prefix",
                            }
                        )

        if violations:
            error_msg = "\n\nSecret reference violations in stg-gke:\n"
            error_msg += "\nKustomize namePrefix='stg-' doesn't auto-update secretKeyRef!\n"
    ...
    def test_keycloak_volume_mounts_dont_override_critical_paths(self):
        ...
        keycloak_deployment = self._find_resource(manifests, "Deployment", "stg-keycloak")

        if not keycloak_deployment:
            pytest.skip("stg-keycloak deployment not found in rendered manifests")
        ...
        if violations:
            error_msg = "\n\nCritical path volume mount violations in stg-keycloak:\n"
    ...
    def test_keycloak_has_required_volume_mounts(self):
        ...
        if missing_mounts:
            error_msg = "\n\nMissing required volume mounts in stg-keycloak:\n"
    ...
    def test_all_deployments_have_valid_images(self):
        ...
        Note: "latest" tags are allowed for stg/dev environments (e.g., stg-latest, dev-latest)
        since we force-pull images anyway. Only production should use pinned versions.
        """
        manifests = self._render_kustomize_manifests()

        INVALID_IMAGE_PATTERNS = [
            ":latest",  # Standalone ":latest" not allowed, but "stg-latest" is OK
            "IMAGE_PLACEHOLDER",
            "YOUR_REGISTRY",
    ...
    def test_environment_labels_use_stg(self):
        """
        Test that all environment labels/annotations use 'stg' not 'staging'.

        RED: Should FAIL initially - kustomization.yaml and namespace.yaml use 'stg'
        GREEN: Should PASS after updating all environment labels to 'stg'

        This ensures consistency after the preview-to-stg rename and prevents
        ...
        if violations:
            error_msg = "\n\nFound 'stg' environment labels (should be 'stg'):\n"
            error_msg += "The preview-to-stg rename requires all environment labels to be updated.\n\n"
            for v in violations:
                error_msg += f"  - {v}\n"
            error_msg += "\nFiles to update:\n"
            error_msg += "  - deployments/overlays/stg-gke/kustomization.yaml\n"
            error_msg += "  - deployments/overlays/stg-gke/namespace.yaml\n"
            pytest.fail(error_msg)
