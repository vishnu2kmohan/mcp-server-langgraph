...
class TestConversationStore:
    ...
    async def test_record_conversation(self, store):
        ...
        await store.record_conversation(
            thread_id="test_thread", user_id=get_user_id("alice"), message_count=5, title="Test Conv"
        )

        metadata = await store.get_conversation("test_thread")
        assert metadata is not None
        assert metadata.thread_id == "test_thread"
        assert metadata.user_id == get_user_id("alice")
    ...
    async def test_update_conversation(self, store):
        ...
        # Create
        await store.record_conversation(thread_id="test_thread", user_id=get_user_id("alice"), message_count=5)

        # Update
        await store.record_conversation(
            thread_id="test_thread", user_id=get_user_id("alice"), message_count=10, title="Updated"
        )
    ...
    async def test_list_user_conversations(self, store):
        ...
        await store.record_conversation(thread_id="alice_1", user_id=get_user_id("alice"), message_count=5)
        await store.record_conversation(thread_id="alice_2", user_id=get_user_id("alice"), message_count=3)
        await store.record_conversation(thread_id="bob_1", user_id=get_user_id("bob"), message_count=7)

        alice_convs = await store.list_user_conversations(get_user_id("alice"))
        assert len(alice_convs) == 2
        assert all(c.user_id == get_user_id("alice") for c in alice_convs)

        bob_convs = await store.list_user_conversations(get_user_id("bob"))
        assert len(bob_convs) == 1
        assert bob_convs[0].user_id == get_user_id("bob")
    ...
    async def test_list_conversations_sorted_by_activity(self, store):
        ...
        await store.record_conversation(thread_id="old", user_id=get_user_id("alice"), message_count=1)
        time.sleep(0.01)
        await store.record_conversation(thread_id="recent", user_id=get_user_id("alice"), message_count=1)

        convs = await store.list_user_conversations(get_user_id("alice"))
        assert len(convs) == 2
    ...
    async def test_search_conversations_by_query(self, store):
        ...
        await store.record_conversation(
            thread_id="project_alpha", user_id=get_user_id("alice"), message_count=5, title="Alpha Project"
        )
        await store.record_conversation(
            thread_id="project_beta", user_id=get_user_id("alice"), message_count=3, title="Beta Release"
        )
        await store.record_conversation(
            thread_id="other", user_id=get_user_id("alice"), message_count=2, title="Something Else"
        )

        # Search for "alpha"
        results = await store.search_conversations(get_user_id("alice"), query="alpha", limit=10)
        assert len(results) == 1
        assert results[0].thread_id == "project_alpha"

        # Search for "project"
        results = await store.search_conversations(get_user_id("alice"), query="project", limit=10)
        assert len(results) == 2

    @pytest.mark.asyncio
    async def test_search_conversations_no_query(self, store):
        """Test search with no query returns most recent"""
        await store.record_conversation(thread_id="conv1", user_id=get_user_id("alice"), message_count=5)
        await store.record_conversation(thread_id="conv2", user_id=get_user_id("alice"), message_count=3)

        results = await store.search_conversations(get_user_id("alice"), query="", limit=10)
        assert len(results) == 2
    ...
    async def test_search_conversations_limit(self, store):
        ...
        for i in range(10):
            await store.record_conversation(thread_id=f"conv{i}", user_id=get_user_id("alice"), message_count=1)

        results = await store.search_conversations(get_user_id("alice"), query="", limit=5)
        assert len(results) == 5

    @pytest.mark.asyncio
    async def test_delete_conversation(self, store):
        """Test deleting conversation"""
        await store.record_conversation(thread_id="test", user_id=get_user_id("alice"), message_count=5)
    ...
    async def test_get_stats(self, store):
        ...
        await store.record_conversation(thread_id="conv1", user_id=get_user_id("alice"), message_count=5)
        await store.record_conversation(thread_id="conv2", user_id=get_user_id("bob"), message_count=3)
    ...
    async def test_search_by_tags(self, store):
        ...
        await store.record_conversation(
            thread_id="tagged", user_id=get_user_id("alice"), message_count=5, tags=["important", "project"]
        )
        await store.record_conversation(thread_id="untagged", user_id=get_user_id("alice"), message_count=3)

        results = await store.search_conversations(get_user_id("alice"), query="important", limit=10)
        assert len(results) == 1
