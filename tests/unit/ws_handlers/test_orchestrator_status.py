"""
Orchestrator Status WebSocket Handler Tests.

TDD tests for the Orchestrator Status WebSocket using the standardized WebSocketBase.

Tests verify:
- Handler extends WebSocketBase correctly
- Method signatures match base class contract
- Subscribe/unsubscribe/get_status flows work
"""
...
class TestOrchestratorStatusHandlerConstruction:
    """Test OrchestratorStatusHandler initialization and base class contract."""
...
@pytest.mark.xdist_group(name="orchestrator_status_ws")
class TestOrchestratorStatusLifecycle:
    """Test Orchestrator Status WebSocket lifecycle methods."""
...
@pytest.mark.xdist_group(name="orchestrator_status_ws")
class TestOrchestratorStatusMessageHandling:
    """Test Orchestrator Status message handling."""
    ...
    @pytest.mark.asyncio
    async def test_handle_get_status(self, mock_broadcaster: MagicMock) -> None:
        """
        GIVEN a get_status message
        WHEN handle_message is called
        THEN it should return status snapshot.
        """
        from mcp_server_langgraph.websocket.handlers.orchestrator_status import (
            OrchestratorStatusHandler,
        )
        ...
        mock_broadcaster.get_status_snapshot.assert_called_once()
