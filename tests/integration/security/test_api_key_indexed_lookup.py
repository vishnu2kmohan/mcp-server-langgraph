...
class TestAPIKeyIndexedLookup:
    ...
    async def test_validate_uses_indexed_search_not_enumeration(self):
        ...
        # Pagination mock: Return user on first call, empty on second to terminate loop
        call_count = 0

        async def mock_search_users(*args, **kwargs):
            nonlocal call_count
            call_count += 1
            # First page: return matching user
                ...
                return [
                    {
                        ...
                        "attributes": {
                            "apiKeys": ["key:test-key-id:$2b$12$hashedkey"],
                        },
                    }
                ]
            # Subsequent pages: empty to stop pagination
            return []

        mock_keycloak.search_users.side_effect = mock_search_users
        mock_keycloak.update_user_attributes = configured_async_mock(return_value=None)

        # Patch bcrypt at import location (where api_keys.py imports it)
        with patch("mcp_server_langgraph.auth.api_keys.bcrypt.checkpw", return_value=True):
            mock_cache = configured_async_mock(return_value=None)
            mock_cache.get.return_value = None
            mock_cache.setex = configured_async_mock(return_value=True)
            manager = APIKeyManager(keycloak_client=mock_keycloak, redis_client=mock_cache, cache_ttl=3600)
            api_key = "test-api-key-12345"
            result = await manager.validate_and_get_user(api_key)

            # Validate search_users was called (pagination approach)
            assert mock_keycloak.search_users.called, "validate_and_get_user should call search_users for user lookup"
            # Validate result structure
            assert result is not None, "Should find matching API key"
            assert result["username"] == "testuser", "Should return correct user"
            assert result["key_id"] == "test-key-id", "Should extract key_id from apiKeys entry"
    ...
    async def test_indexed_lookup_performance_with_large_user_base(self):
        ...
        # Pagination mock: Return user on first call, empty on second
        async def mock_search_users(*args, **kwargs):
            if kwargs.get("first", 0) == 0:
                return [
                    {
                        ...
                        "attributes": {
                            "apiKeys": ["key:perf-key:$2b$12$test"],
                        },
                    }
                ]
            return []
