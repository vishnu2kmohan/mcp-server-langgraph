...
- Cloud DNS zone 'stg-internal' created
...
def skip_if_no_gke_preview_cluster(gke_preview_cluster):
    """
    Skip test if kubectl is not connected to the GKE STG cluster.

    These tests are specific to the GKE STG environment (stg-mcp-server-langgraph-gke)
    and should not run on local Kubernetes clusters (Docker Desktop, Minikube, etc.).
    """
    if not gke_preview_cluster:
        pytest.skip("Not connected to GKE STG cluster. These tests require: gke_*_stg-mcp-server-langgraph-gke context")
    return True
...
class TestDNSConfiguration:
    ...
    def test_dns_zone_exists(self):
        """
        Test that the stg-internal DNS zone exists in GCP.
    ...
    def test_dns_zone_attached_to_correct_vpc(self):
        ...
        # Should be attached to stg-mcp-slg-vpc (the GKE cluster's VPC)
        assert "stg-mcp-slg-vpc" in network_url or "default" in network_url, (
            f"DNS zone attached to wrong network: {network_url}\n"
            f"Expected: stg-mcp-slg-vpc\n"
            f"Fix: gcloud dns managed-zones update stg-internal "
            f"--networks=stg-mcp-slg-vpc --project=vishnu-sandbox-20250310"
        )
...
class TestServiceConfiguration:
    ...
    Note: These tests are specific to the GKE STG cluster and should not run
    ...
    def test_redis_session_service_is_headless_or_external(self, skip_if_no_gke_preview_cluster):
        ...
        Requires connection to the GKE STG cluster.
        """
        result = subprocess.run(
            [
                "kubectl",
    ...
    def test_configmap_uses_dns_names(self, skip_if_no_gke_preview_cluster):
        ...
        result = subprocess.run(
            [
                ...
                "get",
                "configmap",
                "stg-mcp-server-langgraph-config",
                "-n",
                "stg-mcp-server-langgraph",
                "-o",
                "yaml",
