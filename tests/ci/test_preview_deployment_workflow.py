...
- Preview-production parity violations

See: ADR-0055 (Preview Deployment) and GitHub issue #xxx (pods not updating)
"""
...
class TestPreviewProductionParity:
    ...
    def test_preview_has_hpa_configuration_for_autoscaling(self, kustomization_content: dict) -> None:
        ...
        assert "hpa-patch.yaml" in patch_paths, (
            "Preview should have HPA configuration for production parity. Missing hpa-patch.yaml in kustomization patches."
        )
    ...
    def test_preview_has_pdb_configuration_for_availability(self, kustomization_content: dict) -> None:
        ...
        assert "pod-disruption-budget.yaml" in resources, (
            "Preview should have PodDisruptionBudget for production parity. "
    ...
    def test_preview_has_liveness_probe_patches_for_compliance(self, kustomization_content: dict) -> None:
        ...
        for patch in required_probe_patches:
            assert patch in patch_paths, (
                f"Preview should have {patch} for kube-score compliance. Liveness and readiness probes must be differentiated."
            )
...
class TestGracefulShutdown:
    ...
    def test_preview_has_termination_grace_period_configured(self, preview_deployment_patch_content: dict) -> None:
        ...
        """
        spec = preview_deployment_patch_content.get("spec", {}).get("template", {}).get("spec", {})

        termination_grace_period = spec.get("terminationGracePeriodSeconds")

        assert termination_grace_period is not None, "Preview deployment should have terminationGracePeriodSeconds configured"
    ...
    def test_preview_has_prestop_hook_for_draining(self, preview_deployment_patch_content: dict) -> None:
        ...
        """
        containers = preview_deployment_patch_content.get("spec", {}).get("template", {}).get("spec", {}).get("containers", [])

        main_container = next((c for c in containers if c.get("name") == "mcp-server-langgraph"), None)
        assert main_container is not None, "Preview patch should define main container"
