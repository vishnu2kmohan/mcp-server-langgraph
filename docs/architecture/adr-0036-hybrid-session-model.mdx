...
description: 'Architecture Decision Record: 36. Hybrid Session Model for Long-Running Tasks'
icon: 'database'
contentType: "reference"
seoTitle: "36. Hybrid Session Model for Long-Running Tasks - Architecture Decision Record"
seoDescription: "Architecture Decision Record for 36. Hybrid Session Model for Long-Running Tasks. Category: Authentication & Authorization. Part of MCP Server with LangGraph documentation."
keywords:
  - "architecture"
  - "decision"
  - "record"
...
flowchart TB
    ...
    subgraph Redis["Redis Cluster (HA)"]
    ...
    subgraph Refresh["Auto-Refresh Logic"]
        Check{Expires in &lt;5min?}
        Keycloak[Refresh via Keycloak]
        UpdateSession[Update Session]
        SlidingTTL[Extend TTL 30 Days]
    end
    ...
    Check -->|No| Success
    Check -->|Yes| Keycloak
    Keycloak --> UpdateSession
    UpdateSession --> Primary
    UpdateSession --> SlidingTTL
    SlidingTTL --> NewToken
    ...
    classDef refreshStyle fill:#bebada,stroke:#7e5eb0,stroke-width:2px,color:#333
    ...
    class Req,Extract reqStyle
    class Verify,Expired valStyle
    class Primary,Replica1,Replica2,Session redisStyle
    class Check,Keycloak,UpdateSession,SlidingTTL refreshStyle
    class Success,NewToken successStyle
...
- Related ADRs: [ADR-0006](https://github.com/vishnu2kmohan/mcp-server-langgraph/blob/main/adr/adr-0006-session-storage-architecture.md), [ADR-0032](https://github.com/vishnu2kmohan/mcp-server-langgraph/blob/main/adr/adr-0032-jwt-standardization.md), [ADR-0033](https://github.com/vishnu2kmohan/mcp-server-langgraph/blob/main/adr/adr-0033-service-principal-design.md)
