...
description: 'Kong API Gateway can validate JWTs using its built-in JWT plugin, but it currently uses HS256 (symmetric keys) with static secrets. Our architectur...'
icon: 'file-lines'
seoTitle: "ADR-0035: 35. Kong JWT Validation Strategy"
seoDescription: "Architecture Decision Record 0035: Kong API Gateway can validate JWTs using its built-in JWT plugin, but it currently uses HS256 (symmetric keys) with static secrets. Our arch"
keywords: ["ADR", "architecture decision", "design pattern", "MCP Server architecture", "35."]
...
- Related ADRs: [ADR-0031](/architecture/adr-0031-keycloak-authoritative-identity), [ADR-0032](/architecture/adr-0032-jwt-standardization), [ADR-0034](/architecture/adr-0034-api-key-jwt-exchange)
