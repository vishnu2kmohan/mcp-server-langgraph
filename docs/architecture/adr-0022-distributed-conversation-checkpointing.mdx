...
seoTitle: "22. Distributed Conversation Checkpointing for Auto-Scaling - Architecture Decision Record"
seoDescription: "Architecture Decision Record for 22. Distributed Conversation Checkpointing for Auto-Scaling. Category: Data & Storage. Part of MCP Server with LangGraph documentation."
keywords:
  - "architecture"
  - "decision"
  - "record"
...
**Background**: Production incident with stg revision `758b8f744` where an unencoded Redis password containing special characters (`/`, `+`, `=`) caused `ValueError: Port could not be cast to integer value` during `redis.connection.parse_url()`.
...
- **Related ADRs**:
  - [ADR-0006](https://github.com/vishnu2kmohan/mcp-server-langgraph/blob/main/adr/adr-0006-session-storage-architecture.md) - Pluggable session storage (similar pattern)
  - [ADR-0015](https://github.com/vishnu2kmohan/mcp-server-langgraph/blob/main/adr/adr-0015-memory-checkpointing.md) - Original checkpointing decision (superseded)
  - [ADR-0013](https://github.com/vishnu2kmohan/mcp-server-langgraph/blob/main/adr/adr-0013-multi-deployment-target-strategy.md) - Multi-cloud deployment patterns
