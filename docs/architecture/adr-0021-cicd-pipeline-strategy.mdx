...
icon: 'file-lines'
seoTitle: "ADR-0021: 21. CI/CD Pipeline Strategy"
seoDescription: "Architecture Decision Record 0021: Architecture Decision Record: 21. CI/CD Pipeline Strategy"
keywords: ["ADR", "architecture decision", "design pattern", "MCP Server architecture", "21."]
...
flowchart TD
    PR[PR Checks<br/>on pull_request<br/><br/>• Lint Black, isort, flake8<br/>• Type Check mypy<br/>• Unit Tests pytest]

    PR -->|PR approved + merged| CI[CI Pipeline<br/>on push to main<br/><br/>• Lint & Type Check<br/>• Unit Tests all markers<br/>• Integration Tests<br/>• Property Tests Hypothesis<br/>• Contract Tests MCP schema<br/>• Build Docker Image<br/>• Deployment Validation]

    CI -->|tests pass| Quality[Quality Tests<br/>nightly/on-demand<br/><br/>• Mutation Testing mutmut<br/>• Performance Regression Tests<br/>• OpenAPI Breaking Change Detection<br/>• Deployment E2E Tests kind clusters]

    Quality -->|quality gates pass| Security[Security Scan<br/>daily/on-demand<br/><br/>• Bandit Python security linter<br/>• Dependency Audit pip-audit<br/>• SAST Static Application Security<br/>• Container Scanning]

    Security -->|tag pushed| Release[Release Pipeline<br/>on tag v*<br/><br/>• Build & Publish Docker Image<br/>• Build & Publish PyPI Package<br/>• Generate Release Notes<br/>• Create GitHub Release]

    classDef prStyle fill:#8dd3c7,stroke:#2a9d8f,stroke-width:2px,color:#333
    classDef ciStyle fill:#b3de69,stroke:#7cb342,stroke-width:2px,color:#333
    classDef qualityStyle fill:#fdb462,stroke:#e67e22,stroke-width:2px,color:#333
    classDef securityStyle fill:#fb8072,stroke:#c0392b,stroke-width:2px,color:#333
    classDef releaseStyle fill:#bebada,stroke:#8e44ad,stroke-width:2px,color:#333
