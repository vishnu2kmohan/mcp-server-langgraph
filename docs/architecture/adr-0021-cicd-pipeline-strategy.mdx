  ...
  validate-deployments:
    ...
    steps:
      ...
      - name: Validate Helm charts
        run: helm lint deployments/helm/mcp-server-langgraph
...
**Trigger**: Git tag push matching `v*` (e.g., `v2.4.0`)
...
kubectl wait --for=condition=ready pod -l app=mcp-server-langgraph --timeout=300s
```
