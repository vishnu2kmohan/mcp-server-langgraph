...
icon: 'file-lines'
seoTitle: "ADR-0020: 20. Dual MCP Transport Protocol (STDIO + Streamabl"
seoDescription: "Architecture Decision Record 0020: Architecture Decision Record: 20. Dual MCP Transport Protocol (STDIO + StreamableHTTP)"
keywords: ["ADR", "architecture decision", "design pattern", "MCP Server architecture", "20."]
...
flowchart TD
    Agent[Core Agent Logic<br/>agent.py, tools]

    Agent --> STDIO[STDIO Transport<br/>server_stdio.py<br/>JSON-RPC over stdin/stdout]
    Agent --> HTTP[StreamableHTTP<br/>server_streamable.py<br/>HTTP with SSE streaming]

    STDIO --> Desktop[Claude Desktop<br/>Local Tools<br/>Development]
    HTTP --> Cloud[Cloud Deployment<br/>Web Apps<br/>Production]

    classDef agentStyle fill:#8dd3c7,stroke:#2a9d8f,stroke-width:2px,color:#333
    classDef transportStyle fill:#b3de69,stroke:#7cb342,stroke-width:2px,color:#333
    classDef clientStyle fill:#fdb462,stroke:#e67e22,stroke-width:2px,color:#333

    class Agent agentStyle
    class STDIO,HTTP transportStyle
    class Desktop,Cloud clientStyle
```python
