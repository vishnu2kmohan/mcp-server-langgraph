...
description: Architecture Decision Record for worker-scoped resource isolation enabling
  safe parallel test execution with pytest-xdist
icon: flask
contentType: explanation
seoTitle: 'ADR-0052: Pytest-xdist Isolation... - MCP Server LangGraph'
seoDescription: Architecture Decision Record for worker-scoped resource isolation
  enabling safe parallel test execution with pytest-xdist Learn more about MCP Server
  LangGraph.
keywords:
- adr-0052
- pytest-xdist
...
- Memory explosion (217GB VIRT memory usage)
...
3. **Docker Port Range**: Requires ports 9432-10932 (15 workers × 100 ports)
...
- 80 total regression tests (including auth override sanity tests)
- 72/73 tests pass (1 intentional RED test demonstrating incorrect pattern)
- 0 critical validation violations
- All existing tests remain passing

## References

- **Test Guidelines**: `tests/PYTEST_XDIST_BEST_PRACTICES.md` (in source repository)
- **Memory Safety**: `tests/MEMORY_SAFETY_GUIDELINES.md` (in source repository)
- **Worker Utilities**: `tests/utils/worker_utils.py` (in source repository)
- **Regression Tests**: `tests/regression/test_pytest_xdist_*.py` (in source repository)
- OpenAI Codex Findings: Port conflicts, environment pollution, database races
- Commit 079e82e: Initial async/sync mismatch fix
- Commit 8259c81: Core isolation fixes
- Commit 269d7a1: Additional improvements
...
- [ADR-0006: Session Storage Architecture](/architecture/adr-0006-session-storage-architecture) - Uses worker-scoped Redis
- [ADR-0002: OpenFGA Authorization](/architecture/adr-0002-openfga-authorization) - Uses worker-aware fixtures
...
4. **Runtime Validation**: Extend conftest_fixtures_plugin.py to validate bearer_scheme overrides (deferred - static analysis sufficient)
