---
title: "51. Memorystore Redis ExternalName Service with Cloud DNS"
description: 'Architecture Decision Record: 51. Memorystore Redis ExternalName Service with Cloud DNS'
icon: 'database'
contentType: "reference"
seoTitle: "51. Memorystore Redis ExternalName Service with Cloud DNS - Architecture Decision Record"
seoDescription: "Architecture Decision Record for 51. Memorystore Redis ExternalName Service with Cloud DNS. Category: Infrastructure & Deployment. Part of MCP Server with LangGraph documentation."
keywords:
  - "architecture"
  - "decision"
  - "record"
...
3. **Maintain environment portability** - Same manifests work across stg/production with different Redis instances
...
```text
┌─────────────────────────────────────────────────────────────────┐
│ GKE Cluster (stg-mcp-server-langgraph)                      │
│                                                                  │
│  ┌──────────────┐                                               │
│  │   Pod: App   │                                               │
│  │              │                                               │
...
┌─────────────────────────────────────────────┐
│ Cloud DNS (Private Zone: stg.internal)  │
│                                              │
│ redis-session-stg.stg.internal       │
│         ↓                                    │
...
spec:
  type: ExternalName
  externalName: redis-session-stg.stg.internal
```
**Cloud DNS Configuration:**
- Private DNS zone: `stg.internal`
- A record: `redis-session-stg.stg.internal` → Memorystore Redis IP
...
1. **Zero-Downtime Failover**
   - Update Cloud DNS A record: `redis-session-stg.stg.internal` → new IP
   - No manifest changes required
   - No pod restarts needed (DNS TTL-based)

2. **Environment Portability**
   - Same Kustomize manifests work in stg and production
...
1. **Controlled DNS Zone**
   - `stg.internal` is a **private Cloud DNS zone**
...
4. **No User-Controlled Input**
   - DNS name is **hardcoded in manifests** (`redis-session-stg.stg.internal`)
...
- **Residual Risk:** Acceptable for stg and production use
...
- ✅ **Environment Consistency:** Same manifests across stg/production
...
1. **Create Cloud DNS Private Zone** (once per environment)
   ...
   gcloud dns managed-zones create stg-internal \
     ...
     --description="Private DNS for stg GKE services" \
     --visibility=private \
     --networks=gke-vpc
   ```
...
2. **Create DNS A Record** (once per Redis instance)
   ```bash
   gcloud dns record-sets create redis-session-stg.stg.internal \
     --zone=stg-internal \
     --type=A \
     --ttl=300 \
     --rrdatas=10.x.x.x
...
4. **Verify DNS Resolution** (from within a pod)
   ...
   kubectl run -it --rm debug --image=busybox --restart=Never -- \
     nslookup redis-session-stg.stg.internal
   ```
...
1. **Update Cloud DNS A record** to point to new Memorystore Redis instance:
   ```bash
   gcloud dns record-sets update redis-session-stg.stg.internal \
     --zone=stg-internal \
     --type=A \
     --ttl=60 \
...
# - Private Cloud DNS zone (stg.internal) with VPC-only visibility
