...
On 2025-11-12, we experienced pod crashes in the stg environment due to:
...
**❌ WRONG:**
```yaml
# In deployments/overlays/stg-gke/deployment-patch.yaml
env:
  - name: API_KEY
    valueFrom:
      secretKeyRef:
...
env:
  - name: API_KEY
    valueFrom:
      secretKeyRef:
        name: stg-mcp-server-langgraph-secrets  # Includes prefix
        key: api-key
```
...
2. **Add data mapping**:
   ...
   data:
     - secretKey: myNewSecret  # camelCase
       remoteRef:
         key: stg-my-new-secret  # GCP Secret Manager key with prefix
   ```

3. **Create in GCP Secret Manager**:
   ```bash
   echo "secret-value" | gcloud secrets create stg-my-new-secret \
     --data-file=- \
...
- **GCP Secret Manager**: `{prefix}-{kebab-case}` (e.g., `stg-keycloak-client-id`)
...
kustomize build deployments/overlays/stg-gke > /tmp/stg.yaml

# Search for your new keys
grep -A5 "configMapKeyRef" /tmp/stg.yaml
grep -A5 "secretKeyRef" /tmp/stg.yaml

# Check for placeholder values
grep "REPLACE_WITH" /tmp/stg.yaml
```
...
```text
Error: couldn't find key session_cookie_secure in ConfigMap stg-mcp-server-langgraph/stg-mcp-server-langgraph-config
```

**Solution**:
1. Check ConfigMap exists:
   ```bash
   kubectl get configmap -n stg-mcp-server-langgraph
   ```

2. Verify key exists:
   ```bash
   kubectl get configmap stg-mcp-server-langgraph-config -n stg-mcp-server-langgraph -o yaml | grep session_cookie_secure
   ```
...
**Symptom**: Pod can't find secret `mcp-server-langgraph-secrets` but only `stg-mcp-server-langgraph-secrets` exists.

**Solution**:
1. Check ExternalSecret target name:
   ```bash
   kubectl get externalsecret -n stg-mcp-server-langgraph -o yaml
   ```
...
1. Check ExternalSecret status:
   ```bash
   kubectl describe externalsecret stg-mcp-server-langgraph-secrets -n stg-mcp-server-langgraph
   ```
...
3. Verify GCP Secret exists:
   ```bash
   gcloud secrets list --filter="name~stg-" --project=vishnu-sandbox-20250310
   ```
...
5. **Test in stg** before production deployment
...
**Related Issues**: STG pod crashes (2025-11-12)
