---
title: GKE Preview Deployment
description: 'Deploy to GKE stg environment with security best practices'
icon: 'google'
seoTitle: "GKE Preview Deployment - GKE Deployment"
seoDescription: "Deploy MCP Server on GKE: Deploy to GKE stg environment with security best practices"
keywords: ["GKE", "Kubernetes", "deployment", "MCP Server", "cloud deployment"]
contentType: "how-to"
---

### Overview

This guide covers deploying the MCP Server to a production-grade stg environment on Google Kubernetes Engine (GKE) with:
...
<Info>
This is a production-ready stg environment suitable for pre-production testing and validation.
</Info>
...
flowchart TB
    ...
    subgraph GCP["GCP Project (vishnu-sandbox-20250310)"]
        ...
        ArtifactReg["Artifact Registry<br/>Docker images (stg-*)"]
    end
...
- ✅ Preview VPC network (10.1.0.0/20)
...
## Create subnet
gcloud compute networks subnets create stg-gke-subnet \
  --network=stg-vpc \
  --range=10.1.0.0/20 \
  --region=us-central1 \
...
See the [full setup script](https://github.com/vishnu2kmohan/mcp-server-langgraph/blob/main/scripts/gcp/setup-stg-infrastructure.sh) for complete Cloud SQL and Redis setup.
...
```python
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
STG Infrastructure Setup Complete!
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

GitHub Actions Configuration:
  workload_identity_provider: 'projects/PROJECT_NUMBER/locations/global/...'
  service_account: 'mcp-stg-sa@vishnu-sandbox-20250310.iam.gserviceaccount.com'
...
echo -n "sk-ant-YOUR_REAL_KEY" | gcloud secrets versions add stg-anthropic-api-key --data-file=-

## Update Google API key
echo -n "YOUR_REAL_GOOGLE_KEY" | gcloud secrets versions add stg-google-api-key --data-file=-
```
...
Edit `.github/workflows/deploy-stg-gke.yaml` and replace `PROJECT_NUMBER` with your actual project number:
...
```bash
## Get GKE credentials
gcloud container clusters get-credentials stg-mcp-server-langgraph-gke \
  --region=us-central1
...
kubectl port-forward -n stg-mcp-server-langgraph svc/stg-mcp-server-langgraph 8080:80
...
gcloud logging read \
  "resource.type=k8s_container
   resource.labels.cluster_name=stg-mcp-server-langgraph-gke
   resource.labels.namespace_name=mcp-stg" \
  --limit=50 \
  --format=json
```
**Kubectl logs**:
...
```bash
kubectl logs -n stg-mcp-server-langgraph -l app=mcp-server-langgraph --tail=100
```
...
- **Separate VPC**: STG has its own VPC (10.1.0.0/20)
...
```bash
kubectl get networkpolicies -n stg-mcp-server-langgraph
```
#### Workload Identity
...
kubectl describe sa mcp-server-langgraph -n stg-mcp-server-langgraph | grep iam.gke.io
...
gcloud secrets list --filter="name:stg-*"

## View External Secrets sync status
kubectl get externalsecrets -n stg-mcp-server-langgraph
```
...
- Custom metrics: `custom.googleapis.com/mcp-stg/*`
...
<AccordionGroup>
  <Accordion title="Pods not starting" icon="circle-exclamation">
    **Check pod status:**
    ```bash
    kubectl describe pod <pod-name> -n stg-mcp-server-langgraph
    ```
    **Common issues:**
  ...
  <Accordion title="External Secrets not syncing" icon="shield-exclamation">
    **Check ExternalSecret status:**
    ```bash
    kubectl describe externalsecret mcp-stg-secrets -n stg-mcp-server-langgraph
    ```
    **Common fixes:**
  ...
  <Accordion title="Cloud SQL connection fails" icon="database">
    **Check Cloud SQL proxy logs:**
    ```bash
    kubectl logs -n stg-mcp-server-langgraph <pod-name> -c cloud-sql-proxy
    ```
    **Verify connection:**
    ```bash
    # From inside pod
    kubectl exec -it <pod-name> -n stg-mcp-server-langgraph -- \
  ...
  <Accordion title="Network policies blocking traffic" icon="network-wired">
    ...
    **Temporarily disable for testing:**
    ```bash
    kubectl delete networkpolicy <policy-name> -n stg-mcp-server-langgraph
...
kubectl rollout history deployment/stg-mcp-server-langgraph -n stg-mcp-server-langgraph
kubectl rollout undo deployment/stg-mcp-server-langgraph -n stg-mcp-server-langgraph --to-revision=3
...
<CardGroup cols={2}>
  ...
  <Card title="Rightsize Resources" icon="gauge">
    ...
    ```bash
    kubectl top pods -n stg-mcp-server-langgraph
    ```
    Adjust in `deployment-patch.yaml` if needed.
  </Card>

  <Card title="Use Preemptible Nodes" icon="dollar-sign">
    Not recommended for stg, but possible for dev environments.
  </Card>
...
<Check>
**STG Deployment Complete!** Your production-grade stg environment is ready for testing.
</Check>
