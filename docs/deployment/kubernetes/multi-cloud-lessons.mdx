...
- name: Deploy
  run: |
    kubectl create namespace stg-mcp-server-langgraph
...
- name: Create namespace
  run: |
    kubectl create namespace stg-mcp-server-langgraph --dry-run=client -o yaml | kubectl apply -f -
...
2. **IRSA (IAM Roles for Service Accounts)**:
   ...
   eksctl create iamserviceaccount \
     --name=mcp-server-langgraph \
     --namespace=mcp-stg \
     --cluster=stg-mcp-server-langgraph-gke \
...
### Workflow Template (`.github/workflows/deploy-stg-eks.yaml`)

```yaml
name: Deploy to EKS STG
...
env:
  ...
  EKS_CLUSTER: stg-mcp-server-langgraph-gke
  NAMESPACE: mcp-stg
  DEPLOYMENT_NAME: stg-mcp-server-langgraph
  ...
  deploy-stg:
    ...
    steps:
      ...
      - name: Update image tag
        working-directory: deployments/overlays/stg-eks
        run: |
          kustomize edit set image \
            mcp-server-langgraph=ACCOUNT_ID.dkr.ecr.us-east-1.amazonaws.com/mcp-stg:${{ github.sha }}

      # CRITICAL: Use server-side validation
      - name: Validate manifests
        run: |
          echo "Validating manifests..."
          kubectl kustomize deployments/overlays/stg-eks > /tmp/stg-manifests.yaml
          kubectl apply --dry-run=server -f /tmp/stg-manifests.yaml

      # Deploy
      - name: Deploy to stg
        run: |
          kubectl apply -k deployments/overlays/stg-eks
...
### Workflow Template (`.github/workflows/deploy-stg-aks.yaml`)

```yaml
name: Deploy to AKS STG
...
env:
  ...
  RESOURCE_GROUP: mcp-stg-rg
  AKS_CLUSTER: stg-mcp-server-langgraph-gke
  NAMESPACE: mcp-stg
  DEPLOYMENT_NAME: stg-mcp-server-langgraph
...
jobs:
  deploy-stg:
    ...
    steps:
      ...
      # Update image tag
      - name: Update image tag
        working-directory: deployments/overlays/stg-aks
        run: |
          kustomize edit set image \
            mcp-server-langgraph=REGISTRY.azurecr.io/mcp-stg:${{ github.sha }}

      # CRITICAL: Use server-side validation
      - name: Validate manifests
        run: |
          echo "Validating manifests..."
          kubectl kustomize deployments/overlays/stg-aks > /tmp/stg-manifests.yaml
          kubectl apply --dry-run=server -f /tmp/stg-manifests.yaml

      # Deploy
      - name: Deploy to stg
        run: |
          kubectl apply -k deployments/overlays/stg-aks
...
```bash
# 1. Build manifests
kubectl kustomize deployments/overlays/stg-eks > /tmp/eks-manifests.yaml
...
kubectl get namespace stg-mcp-server-langgraph || kubectl create namespace stg-mcp-server-langgraph
...
kubectl auth can-i create roles --namespace stg-mcp-server-langgraph
kubectl auth can-i create rolebindings --namespace stg-mcp-server-langgraph

# 5. Validate resource quotas
kubectl describe resourcequota -n stg-mcp-server-langgraph
```
...
1. Create EKS overlay directory: `deployments/overlays/stg-eks/`
2. Create AKS overlay directory: `deployments/overlays/stg-aks/`
