...
gcloud projects add-iam-policy-binding $PROJECT_ID \
  --member="serviceAccount:langgraph-agent@$PROJECT_ID.iam.gserviceaccount.com" \
...
# Build image
docker build -t gcr.io/$PROJECT_ID/langgraph-agent:latest .
...
spec:
  template:
    spec:
      containers:
      - name: agent
        image: gcr.io/$PROJECT_ID/langgraph-agent:latest
...
```bash
# Clone repository
git clone https://github.com/yourusername/langgraph-mcp-agent
...
helm install langgraph-agent ./helm/langgraph-agent \
  ...
  --set workloadIdentity.serviceAccount=langgraph-agent@$PROJECT_ID.iam.gserviceaccount.com \
...
<AccordionGroup>
  ...
  <Accordion title="Workload Identity not working">
    **Verify setup**:
    ```bash
    # Check annotation
    kubectl describe sa langgraph-agent -n langgraph-agent

    # Test from pod
    kubectl run -it --rm test \
      --image=google/cloud-sdk:slim \
      --serviceaccount=langgraph-agent \
      --namespace=langgraph-agent \
      -- gcloud auth list
    ```
  </Accordion>
