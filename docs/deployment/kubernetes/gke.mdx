...
icon: 'google'
seoTitle: "GKE Deployment: MCP Server on Google Kubernetes Engine"
seoDescription: "Deploy MCP Server to Google Kubernetes Engine (GKE). Workload Identity, Cloud SQL, Vertex AI integration, and GCP-optimized configuration."
keywords: ["GKE deployment", "Google Kubernetes Engine", "GCP", "Workload Identity", "Vertex AI"]
contentType: "how-to"
---
...
spec:
  template:
    spec:
      ...
      containers:
      - name: keycloak
        # IMPORTANT: Use pre-optimized Keycloak image from GHCR
        # Built via .github/workflows/build-keycloak-image.yaml from docker/Dockerfile.keycloak
        # Enables readOnlyRootFilesystem: true (no JIT compilation at runtime)
        ...
        - name: KC_DB
          value: postgres
        # KC_HTTP_RELATIVE_PATH for Traefik/Ingress gateway routing
