---
title: Pod Crash Resolution - 2025-11-12
description: Resolution of critical pod crashes in stg environment affecting Keycloak
  and MCP Server deployments
icon: 'dharmachakra'
contentType: explanation
seoTitle: Pod Crash Resolution - 2025-11-12 - MCP Server LangGraph
seoDescription: Resolution of critical pod crashes in stg environment affecting
  Keycloak and MCP Server deployments Learn more about MCP Server LangGraph.
keywords:
- pod
- crash
- resolution
...
**Severity**: Critical
**Status**: ✅ RESOLVED
**Environment**: stg-mcp-server-langgraph (GKE)

Successfully resolved pod crashes in stg environment affecting Keycloak and MCP Server deployments. Implemented comprehensive TDD-based prevention measures to ensure these issues can never recur.
...
**Solution**:
- Temporarily disabled `readOnlyRootFilesystem` for Keycloak (deployments/overlays/stg-gke/keycloak-patch.yaml:30)
...
**File**: `deployments/overlays/stg-gke/configmap-patch.yaml`
...
#### Secret Keys Added
**File**: `deployments/overlays/stg-gke/external-secrets.yaml`
...
1. **TestConfigMapValidation**
   ...
   - Validates required keys whitelist for stg
...
Validating ConfigMap keys for deployments/overlays/stg-gke...
✓ All ConfigMap keys exist for deployments/overlays/stg-gke
...
=== Kubernetes Configuration Validation ===
✓ All ConfigMap keys exist for deployments/overlays/stg-gke
✓ All ConfigMap keys exist for deployments/overlays/prod-gke
=== Validation Summary ===
✓ All validations passed!
```markdown
...
### Configuration Files
1. `deployments/overlays/stg-gke/configmap-patch.yaml` (+23 lines)
2. `deployments/overlays/stg-gke/external-secrets.yaml` (+40 lines)
3. `deployments/overlays/stg-gke/keycloak-patch.yaml` (modified security context)
4. `deployments/overlays/stg-gke/kustomization.yaml` (+18 lines - ServiceAccount delete patches)
...
1. **a9922e9**: "fix(stg): resolve pod crashes - add missing ConfigMap keys and fix Keycloak readOnlyRootFilesystem"
...
1. **Replace placeholder secrets in GCP Secret Manager**
   ...
   - Update: `stg-keycloak-client-id`, `stg-keycloak-client-secret`
   - Update: `stg-openfga-store-id`, `stg-openfga-model-id`
   - Update: `stg-gdpr-postgres-url`
   - Update: `stg-qdrant-api-key`
   - Update: `stg-infisical-*` credentials

2. **Verify new pods start successfully**
   ```
   kubectl get pods -n stg-mcp-server-langgraph -w
   kubectl logs stg-mcp-server-langgraph-796b46bbf4-xxx
   ```

3. **Scale down old replicasets**
   ```
   kubectl scale deployment stg-mcp-server-langgraph --replicas=3 -n stg-mcp-server-langgraph
   kubectl scale replicaset stg-mcp-server-langgraph-97c744bbd --replicas=0 -n stg-mcp-server-langgraph
   ```diff
