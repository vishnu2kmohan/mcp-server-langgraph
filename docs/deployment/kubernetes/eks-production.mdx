...
icon: 'aws'
seoTitle: "EKS Production Deployment - EKS Deployment"
seoDescription: "Deploy MCP Server on EKS: Complete production deployment guide for AWS EKS with Terraform, IRSA, auto-scaling, and monitoring"
...
flowchart TB
    ...
    class ALB clientStyle
    class API,Pods,CA validationStyle
    class RDS,Redis,SM storageStyle
    class CW,XRAY serviceStyle
```
---
...
<Steps>
  ...
  <Step title="Deploy using Kustomize">
    ```bash
    kubectl apply -k deployments/kubernetes/overlays/prod-eks
    ```
...
<Tip>
**Cost Savings**: Enable spot instances, use single NAT gateway in dev/stg, right-size node types, enable Cluster Autoscaler.
</Tip>
