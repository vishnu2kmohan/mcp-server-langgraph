...
description: 'Deploy to GKE preview environment with security best practices'
icon: 'google'
seoTitle: "GKE Preview Deployment - GKE Deployment"
seoDescription: "Deploy MCP Server on GKE: Deploy to GKE preview environment with security best practices"
...
This guide covers deploying the MCP Server to a production-grade preview environment on Google Kubernetes Engine (GKE) with:
...
<Info>
This is a production-ready preview environment suitable for pre-production testing and validation.
</Info>
    ...
    subgraph GCP["GCP Project (vishnu-sandbox-20250310)"]
        subgraph VPC["Preview VPC (10.1.0.0/20)"]
            subgraph GKE["GKE Autopilot Cluster"]
                ...
                subgraph NS["Namespace: mcp-preview"]
...
## Create subnet
gcloud compute networks subnets create preview-gke-subnet \
  --network=staging-vpc \
  --range=10.1.0.0/20 \
  --region=us-central1 \
...
GitHub Actions Configuration:
  ...
  service_account: 'mcp-preview-sa@vishnu-sandbox-20250310.iam.gserviceaccount.com'
...
Edit `.github/workflows/deploy-preview-gke.yaml` and replace `PROJECT_NUMBER` with your actual project number:
...
## Verify pods
kubectl get pods -n preview-mcp-server-langgraph
```
...
kubectl port-forward -n preview-mcp-server-langgraph svc/preview-mcp-server-langgraph 8080:80
...
gcloud logging read \
  "resource.type=k8s_container
   resource.labels.cluster_name=preview-mcp-server-langgraph-gke
   resource.labels.namespace_name=mcp-preview" \
  --limit=50 \
  --format=json
```
**Kubectl logs**:
...
```bash
kubectl logs -n preview-mcp-server-langgraph -l app=mcp-server-langgraph --tail=100
```
...
kubectl get externalsecrets -n preview-mcp-server-langgraph
```
...
- Custom metrics: `custom.googleapis.com/mcp-preview/*`
...
<AccordionGroup>
  <Accordion title="Pods not starting" icon="circle-exclamation">
    **Check pod status:**
    ```bash
    kubectl describe pod <pod-name> -n preview-mcp-server-langgraph
    ```
    **Common issues:**
  ...
  <Accordion title="External Secrets not syncing" icon="shield-exclamation">
    **Check ExternalSecret status:**
    ```bash
    kubectl describe externalsecret mcp-preview-secrets -n preview-mcp-server-langgraph
    ```
    **Common fixes:**
  ...
  <Accordion title="Cloud SQL connection fails" icon="database">
    **Check Cloud SQL proxy logs:**
    ```bash
    kubectl logs -n preview-mcp-server-langgraph <pod-name> -c cloud-sql-proxy
    ```
    **Verify connection:**
    ```bash
    # From inside pod
    kubectl exec -it <pod-name> -n preview-mcp-server-langgraph -- \
  ...
  <Accordion title="Network policies blocking traffic" icon="network-wired">
    ...
    **Temporarily disable for testing:**
    ```bash
    kubectl delete networkpolicy <policy-name> -n preview-mcp-server-langgraph
...
kubectl rollout undo deployment/preview-mcp-server-langgraph -n preview-mcp-server-langgraph --to-revision=3
...
<CardGroup cols={2}>
  ...
  <Card title="Rightsize Resources" icon="gauge">
    ...
    ```bash
    kubectl top pods -n preview-mcp-server-langgraph
    ```
...
<Check>
**Staging Deployment Complete!** Your production-grade preview environment is ready for testing.
</Check>
