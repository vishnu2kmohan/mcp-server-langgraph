...
<AccordionGroup>
  ...
  <Accordion title="Multi-Environment Control">
    ...
    **Implementation**:
    ...
    - STG: Require stg attestor (audit mode)
...
spec:
  defaultAdmissionRule:
    ...
    requireAttestationsBy:
      - projects/PROJECT_ID/attestors/stg-attestor
...
<CodeGroup>
```yaml .github/workflows/deploy-prod-gke.yaml
...
jobs:
  build-sign-deploy:
    ...
    steps:
      ...
      - name: Build Image
        run: |
          docker build -t us-central1-docker.pkg.dev/PROJECT_ID/mcp-prod/mcp-server-langgraph:${{ github.ref_name }} .

      - name: Push Image
        run: |
          docker push us-central1-docker.pkg.dev/PROJECT_ID/mcp-prod/mcp-server-langgraph:${{ github.ref_name }}

      - name: Sign Image
        run: |
          IMAGE_URL="us-central1-docker.pkg.dev/PROJECT_ID/mcp-prod/mcp-server-langgraph:${{ github.ref_name }}"
          IMAGE_DIGEST=$(gcloud artifacts docker images describe $IMAGE_URL --format='get(image_summary.digest)')

          # Create attestation
          gcloud beta container binauthz attestations sign-and-create \
            --artifact-url="us-central1-docker.pkg.dev/PROJECT_ID/mcp-prod/mcp-server-langgraph@${IMAGE_DIGEST}" \
...
gcloud container binauthz attestations list \
  --artifact-url="us-central1-docker.pkg.dev/PROJECT_ID/mcp-prod/mcp-server-langgraph@sha256:ABC123" \
  --attestor=production-attestor \
  --attestor-project=PROJECT_ID
```
...
# STG key (different key ring)
stg-binauthz/attestor-key
...
<Tabs>
  <Tab title="Control Objectives Met">
    ...
    - ✅ **CC6.6**: Logical access segregation
      ...
      - Production attestor separate from dev/stg
...
<Steps>
  ...
  <Step title="3. Enable Audit Mode (STG)">
    ```bash
    # STG cluster - audit mode
...
<CardGroup cols={2}>
  ...
  <Card title="CI/CD Pipeline" icon="github" href="https://github.com/vishnu2kmohan/mcp-server-langgraph/blob/main/.github/workflows/deploy-prod-gke.yaml">
