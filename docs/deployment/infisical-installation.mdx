...
icon: 'shield-halved'
seoTitle: "Infisical Installation Guide - Deployment Guide"
seoDescription: "Deploy MCP Server: Complete guide for installing and using Infisical secrets management with MCP Server LangGraph. Infisical is an optional dependency for"
...
uv pip install --no-index --find-links=wheels/py$(python3 -c 'import sys; print(f"{sys.version_info.major}.{sys.version_info.minor}")') infisical-python

## Or specify version
uv pip install --no-index --find-links=wheels/py3.12 infisical-python
```
...
uv pip install 'infisical-python>=2.1.7,`<2.3.6`'
