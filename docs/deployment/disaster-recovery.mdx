...
velero backup create langgraph-backup \
  --include-namespaces langgraph-agent
...
kubectl get secrets -n langgraph-agent -o yaml > secrets-backup.yaml
...
velero restore create dr-test-restore \
  ...
  --namespace-mappings langgraph-agent:langgraph-dr-test
...
kubectl scale deployment langgraph-agent --replicas=10
