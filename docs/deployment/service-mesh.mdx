...
seoTitle: "Service Mesh (Anthos) - Deployment Guide"
seoDescription: "Deploy MCP Server: Deploy Anthos Service Mesh on GKE for service-to-service mTLS, traffic management, and observability"
...
<AccordionGroup>
  ...
  <Accordion title="Multi-Cluster Mesh">
    **Scenario**: Services across dev, stg, prod clusters
...
```mermaid
flowchart TB
    subgraph "GKE Cluster (prod-mcp-server-langgraph-gke)"
...
<Steps>
  <Step title="Enable APIs & Fleet Registration">
    ...
    ./deployments/service-mesh/anthos/setup-anthos-service-mesh.sh \
      PROJECT_ID prod-mcp-server-langgraph-gke us-central1
    ```
  ...
  <Step title="Enable Sidecar Injection">
    ...
    kubectl label namespace prod-mcp-server-langgraph istio-injection=enabled
  ...
  <Step title="Verify Sidecars Injected">
    ...
    # Should show:
    # NAME                                   READY   STATUS
    # prod-mcp-server-langgraph-...    2/2     Running
    #                                        ^^^
    #                                    app + sidecar

    # Describe pod to see istio-proxy container
    kubectl describe pod POD_NAME -n prod-mcp-server-langgraph | grep istio-proxy
    ```
  </Step>
  ...
  <Step title="Verify mTLS">
    ...
    istioctl proxy-config secret -n prod-mcp-server-langgraph POD_NAME
...
spec:
  ...
  - from:
    - source:
        principals: ["cluster.local/ns/mcp-prod/sa/mcp-server"]
    to:
    - operation:
        methods: ["GET", "POST"]
        paths: ["/api/*"]
...
<Tabs>
  <Tab title="Request Rate">
    ...
    rate(istio_requests_total{
      ...
      destination_workload_namespace="mcp-prod"
    }[1m])
    ```
  </Tab>
...
<Steps>
  <Step title="Register All Clusters">
    ...
    # STG cluster
    gcloud container fleet memberships register mcp-stg-membership \
      --gke-cluster=us-central1/mcp-stg-gke \
      --project=PROJECT_ID
  ...
  <Step title="Enable Mesh for All">
    ...
    gcloud container fleet mesh update \
      ...
      --memberships=mcp-dev-membership,mcp-stg-membership,mcp-prod-membership \
      --project=PROJECT_ID
    ```
  </Step>
  ...
  <Step title="Configure Cross-Cluster Service Discovery">
    ...
    kind: ServiceEntry
    metadata:
      name: external-stg-service
      namespace: prod-mcp-server-langgraph
    spec:
      hosts:
      - mcp-server.mcp-stg.svc.cluster.local
...
**Use case**: Production can call stg services for integration testing
...
<AccordionGroup>
  <Accordion title="Sidecar not injected">
    ...
    **Checks**:
    ...
    kubectl get namespace prod-mcp-server-langgraph --show-labels
  ...
  <Accordion title="mTLS connection failure">
    ...
    **Checks**:
    ...
    kubectl get peerauthentication -n prod-mcp-server-langgraph

    # Check DestinationRule
    kubectl get destinationrule -n prod-mcp-server-langgraph

    # Verify certificates
    istioctl proxy-config secret POD_NAME -n prod-mcp-server-langgraph
    ```
...
<Steps>
  ...
  <Step title="Enable Sidecar Injection">
    ```bash
    kubectl label namespace prod-mcp-server-langgraph istio-injection=enabled
    kubectl rollout restart deployment -n prod-mcp-server-langgraph
    ```
  </Step>
