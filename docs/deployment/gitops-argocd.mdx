...
description: 'Implement declarative, Git-driven deployments for MCP Server across dev, stg, and production GKE clusters'
icon: 'code-branch'
seoTitle: "GitOps with ArgoCD - Deployment Guide"
seoDescription: "Deploy MCP Server: Implement declarative, Git-driven deployments for MCP Server across dev, stg, and production GKE clusters"
...
<CardGroup cols={2}>
  <Card title="Multi-Cluster" icon="layer-group">
    Manage dev, stg, prod from one ArgoCD instance
  </Card>
...
flowchart TB
    subgraph "Git Repository"
        ...
        B[deployments/overlays/prod-gke/]
        C[deployments/overlays/stg-gke/]
...
<Steps>
  ...
  <Step title="Add GKE Clusters">
    <Tabs>
      <Tab title="Using argocd CLI">
        ...
        argocd cluster add gke_YOUR_PROJECT_ID_us-central1_mcp-stg-gke --name gcp-stg
        argocd cluster add gke_YOUR_PROJECT_ID_us-central1_prod-mcp-server-langgraph-gke --name gcp-production
        ```
      </Tab>
      ...
      <Tab title="Using Secrets (Declarative)">
        ...
        kubectl config view --minify --flatten \
          --context=gke_YOUR_PROJECT_ID_us-central1_prod-mcp-server-langgraph-gke > prod-cluster.yaml
  ...
  <Step title="Deploy ApplicationSet">
    ...
    Creates 3 Applications (dev, stg, prod) from one definition.
  </Step>
...
2. mcp-server-gcp-stg
   - Cluster: gcp-stg
   - Path: deployments/overlays/stg-gke
   - Namespace: mcp-stg

3. mcp-server-gcp-production
   - Cluster: gcp-production
   - Path: deployments/overlays/prod-gke
   - Namespace: mcp-prod
```
</CodeGroup>
...
<Tabs>
  <Tab title="Automated (Recommended)">
    ...
    **Best for**: STG, production (strict GitOps)
  </Tab>
...
<Steps>
  ...
  <Step title="Annotate Application">
    ...
    kind: Application
    metadata:
      name: mcp-server-prod-gke
      namespace: argocd
      annotations:
        # Image to watch
        argocd-image-updater.argoproj.io/image-list: |
          mcp-server=us-central1-docker.pkg.dev/PROJECT_ID/mcp-prod/mcp-server-langgraph
...
<Steps>
  ...
  <Step title="2. Promote to STG (Manual Sync)">
    ```bash
    # Option 1: argocd CLI
    argocd app sync mcp-server-gcp-stg

    # Option 2: UI
    # Click "Sync" in ArgoCD UI for stg app
    ```
              ...
              - name: Update production overlay
                run: |
                  ...
                  kustomize edit set image \
                    mcp-server=us-central1-docker.pkg.dev/PROJECT_ID/mcp-prod/mcp-server-langgraph:${{ github.event.inputs.version }}
...
data:
  policy.csv: |
    ...
    # DevOps: full access to stg/prod
...
<CardGroup cols={2}>
  ...
  <Card title="CI/CD Pipeline" icon="code-branch" href="https://github.com/vishnu2kmohan/mcp-server-langgraph/blob/main/.github/workflows/deploy-prod-gke.yaml">
