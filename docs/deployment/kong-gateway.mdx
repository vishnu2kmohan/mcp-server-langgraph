...
curl -X POST http://localhost:8001/services \
  --data name=langgraph-agent \
  --data url=http://langgraph-agent:8000
...
curl -X POST http://localhost:8001/services/langgraph-agent/routes \
  --data paths[]=/message \
  --data methods[]=POST
...
```bash
# Enable JWT plugin
curl -X POST http://localhost:8001/services/langgraph-agent/plugins \
  --data name=jwt \
...
# Example metrics:
# kong_http_requests_total{service="langgraph-agent",route="message",code="200"} 1234
# kong_latency_ms{type="request",service="langgraph-agent"} 45.2
# kong_bandwidth_bytes{type="ingress",service="langgraph-agent"} 1024000
```
...
<AccordionGroup>
  ...
  <Accordion title="502 Bad Gateway">
    **Debug**:
    ...
    # Check service
    curl http://localhost:8001/services/langgraph-agent

    # Test upstream directly
    curl http://langgraph-agent:8000/health
    ```

    **Fix**:
    ```bash
    # Update service URL
    curl -X PATCH http://localhost:8001/services/langgraph-agent \
      --data url=http://correct-host:8000

    # Check network connectivity
    kubectl exec -it kong-0 -n kong -- curl http://langgraph-agent:8000/health
    ```
  </Accordion>
