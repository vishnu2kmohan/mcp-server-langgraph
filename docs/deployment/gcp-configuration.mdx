...
seoTitle: GCP Configuration Guide - MCP Server LangGraph
seoDescription: Google Cloud Platform settings for CI/CD deployment workflows Learn
...
The deployment workflows (`deploy-stg-gke.yaml`, `deploy-prod-gke.yaml`) require GCP configuration for:
...
```bash
gcloud container clusters create stg-mcp-server-langgraph-gke \
  --project=$PROJECT_ID \
  --region=$GCP_REGION \
  --enable-autoscaling \
  --min-nodes=1 \
  ...
  --labels=environment=stg,managed-by=terraform
```

#### Production Cluster

```bash
gcloud container clusters create prod-mcp-server-langgraph-gke \
  --project=$PROJECT_ID \
  --region=$GCP_REGION \
  --enable-autoscaling \
  --min-nodes=2 \
...
| `GKE_STG_CLUSTER` | `stg-mcp-server-langgraph-gke` | STG GKE cluster name |
| `GKE_PRODUCTION_CLUSTER` | `prod-mcp-server-langgraph-gke` | Production GKE cluster name |
...
# Cluster Names
echo "GKE_STG_CLUSTER: stg-mcp-server-langgraph-gke"
echo "GKE_PRODUCTION_CLUSTER: prod-mcp-server-langgraph-gke"
```
---
...
env:
  ...
  GKE_CLUSTER: ${{ secrets.GKE_STG_CLUSTER }}
...
```bash
# Via GitHub UI: Actions → Deploy STG GKE → Run workflow

# Or via gh CLI:
gh workflow run deploy-stg-gke.yaml
```
---
...
Use different service accounts for stg vs production:

```bash
# STG service account
gcloud iam service-accounts create github-actions-stg \
  --project=$PROJECT_ID
...
3. **Test STG Deployment** - Trigger manual workflow run
