...
flowchart LR
    ...
    subgraph STG["STG"]
        ...
        StageK8s["values-stg.yaml"]
    end

    subgraph Prod["Production"]
        ProdK8s["values-prod-gke.yaml"]
        ProdNet["mcp-network"]
    end

    Dev -->|"CI Tests"| Test
    Test -->|"Merge to main"| STG
    STG -->|"Release tag"| Prod
...
|-------------|---------|-------------------|-------------------|--------------|
| **Production** | Live production workloads | `docker-compose.yml` | `deployments/helm/values-prod-gke.yaml` | `mcp-network` |
