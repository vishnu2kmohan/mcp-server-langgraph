...
description: 'Configure dev, stg, and production environments with Terraform for consistent, cost-optimized infrastructure'
icon: 'layer-group'
seoTitle: "Multi-Environment Strategy - Infrastructure as Code"
seoDescription: "MCP Server infrastructure: Configure dev, stg, and production environments with Terraform for consistent, cost-optimized infrastructure"
...
MCP Server uses a **multi-environment Terraform strategy** to maintain consistent infrastructure across development, stg, and production while optimizing costs and managing risk.

<CardGroup cols={3}>
  <Card title="Development" icon="flask">
    Cost-optimized for testing
  </Card>
  <Card title="STG" icon="vial">
...
| Component | Development | STG | Production |
|-----------|-------------|---------|------------|
...
│   ├── terraform.tfvars     # STG-specific values (larger sizes)
│   ├── backend.tf           # GCS backend (stg prefix)
│   └── outputs.tf
│
└── gcp-prod/
    ├── main.tf              # Same modules as dev/stg
    ├── variables.tf         # Same variables
...
<AccordionGroup>
  <Accordion title="No Read Replicas">
    ...
    **STG**: No read replicas (cost savings, same testing value).
  </Accordion>

  <Accordion title="Binary Auth in Audit Mode">
    **Production**: Binary Authorization **enforcing** (blocks unsigned images).

    **STG**: **Audit mode** (logs denials, doesn't block). Allows testing image signing without risk.
  </Accordion>

  <Accordion title="Smaller Database">
    **Production**: db-custom-4-15360 (4 vCPU).

    **STG**: db-custom-2-7680 (2 vCPU). Sufficient for load testing without production traffic.
  </Accordion>
</AccordionGroup>
...
<Tabs>
  ...
  <Tab title="STG">
    ```hcl terraform/environments/gcp-stg/backend.tf
    terraform {
      backend "gcs" {
        bucket = "mcp-langgraph-terraform-state"
        prefix = "environments/stg"
      }
    }
    ```
  </Tab>
...
|----------|-----|---------|------|
| `project_id` | mcp-dev | mcp-stg | mcp-prod |
| `cluster_name` | mcp-dev-gke | mcp-stg-gke | prod-mcp-server-langgraph-gke |
...
<Steps>
  <Step title="Unit Tests (Terraform Validate)">
    ```bash
    for env in gcp-dev gcp-stg gcp-prod; do
...
kubectl top pods -n stg-mcp-server-langgraph --containers
