...
flowchart TB
    ...
    class ALB,NAT clientStyle
    class CP,NG1,NG2,NG3 validationStyle
    class RDS,Redis,S3,DDB storageStyle
    class KMS,SM serviceStyle
    class VPCEndpoints,CW secondaryStyle
```
## Modules Overview
...
| **Total (Single NAT)** | **$59.00** | Development/stg |

<Tip>
**Cost Savings**: Use single NAT gateway in dev/stg to save ~$65/month. VPC endpoints save ~70% on data transfer costs, paying for themselves with moderate traffic.
</Tip>
...
<Tip>
**Cost Optimization**: Use `db.t3.small` for dev/stg ($60.28/month). For production, `db.t3.medium` provides good balance of performance and cost.
</Tip>
...
<Tabs>
  ...
  <Tab title="Standard Mode (Dev/STG)">
    **Configuration**:
    ...
    **Use case**: Development, stg, low-traffic production
  </Tab>
</Tabs>
...
<Tip>
**Cost Optimization**: Use Standard mode with `cache.t4g.micro` for dev/stg ($24.82/month for 2 nodes).
</Tip>
...
<AccordionGroup>
  ...
  <Accordion title="Single NAT Gateway (non-production)">
    ```hcl
    single_nat_gateway = true  # Dev/stg only
    ```
  ...
  <Accordion title="Right-size RDS instances">
    ...
    - **STG**: db.t3.medium ($60.28/month, Single-AZ)
    - **Prod**: db.t3.medium Multi-AZ ($120.56/month)

    **Savings**: $90.42/month using Single-AZ in stg
  </Accordion>
