...
<Info>
  This is a **one-time setup** per GCP project. Once complete, all Terraform environments (dev, stg, prod) will use these buckets.
</Info>
...
<Tabs>
  ...
  <Tab title="Alternative (Bucket-per-Environment)">
    **Separate buckets**:
    ...
    mcp-langgraph-stg-terraform-state/
    └── default.tfstate
