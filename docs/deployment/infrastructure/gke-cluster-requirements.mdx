...
seoDescription: "Learn how to create and configure GKE Autopilot clusters for stg and production CI/CD deployments"
...
**Name:** `stg-mcp-server-langgraph-gke`
...
**Name:** `prod-mcp-server-langgraph-gke`
**Location:** `us-central1`
**Project:** `vishnu-sandbox-20250310`
**Type:** GKE Autopilot
...
- ✅ GKE cluster: `stg-mcp-server-langgraph-gke` in `us-central1`
- ✅ Service account: `github-actions-stg@vishnu-sandbox-20250310.iam.gserviceaccount.com`
- ✅ Artifact Registry: `mcp-stg` repository
...
This workflow requires:
- ⚠️  GKE cluster: `prod-mcp-server-langgraph-gke` in `us-central1`
- ✅ Service account: `github-actions-production@vishnu-sandbox-20250310.iam.gserviceaccount.com`
- ✅ Artifact Registry: `mcp-prod` repository
...
**Symptom:**
```yaml
Error: NOT_FOUND: projects/vishnu-sandbox-20250310/locations/us-central1/clusters/stg-mcp-server-langgraph-gke not found
```
...
**Symptom:**
```yaml
Error: github-actions-stg does not have permission to access cluster
```
...
gcloud projects get-iam-policy vishnu-sandbox-20250310 \
  ...
  --filter="bindings.members:github-actions-stg*"
```
...
```bash
# Preview
gcloud artifacts repositories create mcp-stg \
  --repository-format=docker \
  --location=us-central1 \
  --project=vishnu-sandbox-20250310

# Production
gcloud artifacts repositories create mcp-prod \
  --repository-format=docker \
  --location=us-central1 \
  --project=vishnu-sandbox-20250310
```
...
- GKE clusters: ✅ Created (stg-mcp-server-langgraph-gke)
