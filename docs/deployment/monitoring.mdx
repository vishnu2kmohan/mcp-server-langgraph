...
<Tabs>
  ...
  <Tab title="Kubernetes ConfigMap">
    **Provision all dashboards automatically:**

    ```bash
    # Create ConfigMap with all v2.1.0 dashboards
    kubectl create configmap grafana-dashboards \
      --from-file=monitoring/grafana/dashboards/authentication.json \
      --from-file=monitoring/grafana/dashboards/openfga.json \
      --from-file=monitoring/grafana/dashboards/llm-performance.json \
      --from-file=monitoring/grafana/dashboards/langgraph-agent.json \
      --from-file=monitoring/grafana/dashboards/security.json \
      --from-file=monitoring/grafana/dashboards/keycloak.json \
      --from-file=monitoring/grafana/dashboards/redis-sessions.json \
      -n monitoring
    ...
    grafana:
      ...
      dashboards:
        langgraph-agent:
          ...
          overview:
            file: monitoring/grafana/dashboards/langgraph-agent.json
          security:
...
<CardGroup cols={2}>
  ...
  <Card title="Alerts" icon="bell" href="/deployment/alerting">
    Alert configuration
  </Card>
  <Card title="Security Monitoring" icon="shield" href="/security/monitoring">
