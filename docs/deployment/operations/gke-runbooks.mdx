...
icon: 'book-medical'
seoTitle: "GKE Operational Runbooks - Operations Guide"
seoDescription: "Operations runbook: Day-2 operations, incident response, and maintenance procedures for MCP Server on GKE Autopilot"
...
<Steps>
  ...
  <Step title="Review Recent Events">
    ```bash
    kubectl get events -n prod-mcp-server-langgraph \
...
<Steps>
  <Step title="Immediate Assessment">
    ...
    # Get pod logs
    kubectl logs -n prod-mcp-server-langgraph -l app=mcp-server-langgraph --tail=100

    # Describe failing pods
    kubectl describe pod POD_NAME -n prod-mcp-server-langgraph
    ```
  </Step>
  ...
  <Step title="Quick Fixes">
    <Tabs>
      <Tab title="Restart Deployment">
        ```bash
        kubectl rollout restart deployment/prod-mcp-server-langgraph \
  ...
  <Step title="Verify Recovery">
    ```bash
    kubectl get pods -n prod-mcp-server-langgraph
    kubectl exec -it -n prod-mcp-server-langgraph POD_NAME -- curl http://localhost:8000/health/live
    ```
  </Step>
...
<Steps>
  <Step title="Check Resource Usage">
    ```bash
    kubectl top pods -n prod-mcp-server-langgraph
    kubectl top nodes
    ```
  </Step>
  ...
  <Step title="Check HPA Status">
    ```bash
    kubectl get hpa -n prod-mcp-server-langgraph -o yaml
    ```
  </Step>
  ...
  <Step title="Temporary Scale Up">
    ...
    # Monitor improvement
    watch -n 5 'kubectl top pods -n prod-mcp-server-langgraph'
    ```
  </Step>
</Steps>
...
<AccordionGroup>
  <Accordion title="Connection Refused">
    **Check Cloud SQL Proxy**:
    ```bash
    kubectl logs -n prod-mcp-server-langgraph POD_NAME -c cloud-sql-proxy --tail=50
    ```
    ...
    **Restart Proxy**:
    ```bash
    kubectl rollout restart deployment/prod-mcp-server-langgraph -n prod-mcp-server-langgraph
    ```
  </Accordion>
  ...
  <Accordion title="Too Many Connections">
    **Check Connection Count**:
    ```bash
    kubectl port-forward -n prod-mcp-server-langgraph svc/prod-mcp-server-langgraph 5432:5432 &
...
<Steps>
  ...
  <Step title="Verify Deployment">
    ```bash
    kubectl rollout status deployment/prod-mcp-server-langgraph -n prod-mcp-server-langgraph
    kubectl get pods -n prod-mcp-server-langgraph
    ```
  </Step>
</Steps>
...
<Tabs>
  ...
  <Tab title="Specific Revision">
    ```bash
    # List revisions
    kubectl rollout history deployment/prod-mcp-server-langgraph \
      -n prod-mcp-server-langgraph
...
<Steps>
  ...
  <Step title="Pods Auto-Restart">
    ...
    Monitor:
    ```bash
    kubectl get pods -n prod-mcp-server-langgraph -w
    ```
  </Step>
</Steps>
...
<AccordionGroup>
  <Accordion title="View Application Logs">
    ...
    ```bash Real-time
    kubectl logs -f -n prod-mcp-server-langgraph \
    ...
    kubectl logs POD_NAME -n prod-mcp-server-langgraph -c mcp-server-langgraph
    kubectl logs POD_NAME -n prod-mcp-server-langgraph -c cloud-sql-proxy
    ```
  ...
  <Accordion title="Update Configuration">
    ```bash
    # Edit ConfigMap
    kubectl edit configmap prod-mcp-server-langgraph-config \
      -n prod-mcp-server-langgraph
  ...
  <Accordion title="Connect to Database">
    <Tabs>
      ...
      <Tab title="kubectl Port Forward">
        ```bash
        kubectl port-forward -n prod-mcp-server-langgraph \
          svc/prod-mcp-server-langgraph 5432:5432 &
