...
<Steps>
  <Step title="Identify the issue">
    ...
    # Check pod events
    kubectl describe pod mcp-server-xxxx -n mcp-server-langgraph | tail -20
  ...
  <Step title="Fix based on cause">
    <Tabs>
      ...
      <Tab title="IRSA Issues">
        ...
        kubectl describe pod mcp-server-xxxx -n mcp-server-langgraph | grep AWS_ROLE_ARN
        ```
      </Tab>
      ...
      <Tab title="Resource Limits">
        ...
        kubectl describe pod mcp-server-xxxx -n mcp-server-langgraph | grep -A 5 "Last State"
