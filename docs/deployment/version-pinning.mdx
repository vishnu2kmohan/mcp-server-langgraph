...
STG environments **SHOULD** use explicit version tags with environment prefix:
...
| `deployments/kustomize/overlays/stg/kustomization.yaml` | stg-2.4.0 | ✅ Pinned | STG |
| `deployments/kustomize/overlays/prod/kustomization.yaml` | v2.4.0 | ✅ Pinned | Production |
| **Cloud Run** |
...
## Git SHA (development/stg)
mcp-server-langgraph:dev-a1b2c3d
mcp-server-langgraph:stg-a1b2c3d
...
mcp-server-langgraph:stg-2.4.0
...
images:
  - name: mcp-server-langgraph
    newTag: stg-2.4.0  # Explicit version
```
...
- [ ] STG tested with exact production tag
...
- ✅ Test with production tags in stg
