...
<Steps>
  ...
  <Step title="Start services">
    ```bash
    docker compose up -d
    # Note: Use 'docker-compose' (with hyphen) if using v1
    ```

    First build takes 2-3 minutes. Subsequent starts are under 30 seconds.

    <Note>
      **Docker Compose Version**: This guide uses v2 syntax (`docker compose`). If you have the legacy v1 installed, use `docker-compose` (with hyphen) instead.
    </Note>
  </Step>
