...
## Update deployment
kubectl patch deployment prod-mcp-server-langgraph \
  -n mcp-prod \
  --type='json' \
...
**Stop dev/stg environments after hours**:
...
<AccordionGroup>
  <Accordion title="Compute">
    ...
    <Check>Stop dev/stg environments after hours</Check>
  </Accordion>
  ...
  <Accordion title="Network">
    ...
    <Check>Use single NAT gateway in dev/stg</Check>
...
<CardGroup cols={2}>
  ...
  <Card title="Multi-Environment" icon="layer-group" href="/deployment/infrastructure/multi-environment">
    Dev/stg cost optimization strategies
  </Card>
