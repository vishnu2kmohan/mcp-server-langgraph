...
# Watch scaling
kubectl get hpa -n langgraph-agent --watch
```
...
data:
  config.yaml: |
    rules:
    - seriesQuery: 'http_requests_total{namespace="langgraph-agent"}'
      resources:
        template: <<.Resource>>
      name:
        matches: "^(.*)_total"
...
```bash
kubectl describe vpa langgraph-agent-vpa -n langgraph-agent
```
...
```bash
# Watch pods scaling
watch kubectl get pods -n langgraph-agent

# Watch HPA
watch kubectl get hpa -n langgraph-agent
...
kubectl get vpa langgraph-agent-vpa -o yaml
      ...
      template:
        spec:
          ...
          - name: kubectl
            ...
            - /bin/sh
            - -c
            - kubectl scale deployment langgraph-agent --replicas=1
...
```promql
# Replica count
count(kube_pod_info{namespace="langgraph-agent", pod=~"langgraph-agent.*"})

# CPU utilization
rate(container_cpu_usage_seconds_total{namespace="langgraph-agent"}[5m])

# Memory utilization
container_memory_working_set_bytes{namespace="langgraph-agent"}

# Request rate
rate(http_requests_total{namespace="langgraph-agent"}[5m])

# HPA desired replicas
kube_horizontalpodautoscaler_status_desired_replicas{namespace="langgraph-agent"}
```
...
- name: scaling
  ...
  - alert: HPAMaxedOut
    expr: |
      kube_horizontalpodautoscaler_status_desired_replicas{namespace="langgraph-agent"}
      >=
      kube_horizontalpodautoscaler_spec_max_replicas{namespace="langgraph-agent"}
    for: 5m
    annotations:
  ...
  - alert: HighCPU
    expr: |
      avg(rate(container_cpu_usage_seconds_total{namespace="langgraph-agent"}[5m])) > 0.8
    for: 2m
    annotations:
  ...
  - alert: HighMemory
    expr: |
      avg(container_memory_working_set_bytes{namespace="langgraph-agent"})
      /
...
<AccordionGroup>
  <Accordion title="HPA not scaling">
    ```bash
    # Check HPA status
    kubectl describe hpa langgraph-agent -n langgraph-agent
