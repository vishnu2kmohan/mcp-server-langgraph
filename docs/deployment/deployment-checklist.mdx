...
seoDescription: Complete infrastructure deployment checklist for stg, production, and development environments. Ensure reproducible and idempotent deployments with validation steps.
keywords:
...
**Status:** ✅ Validated with stg rebuild
...
! grep -r "mcp-stg-cluster\|mcp-prod-gke\|mcp-dev-cluster" \
...
grep -r "stg-mcp-server-langgraph-gke\|prod-mcp-server-langgraph-gke" \
...
# Should show:
# - GKE_STG_CLUSTER: stg-mcp-server-langgraph-gke
# - GKE_PROD_CLUSTER: prod-mcp-server-langgraph-gke
# - STG_NAMESPACE: stg-mcp-server-langgraph
# - PRODUCTION_NAMESPACE: prod-mcp-server-langgraph
```
...
**Last Validated:** 2025-11-12 (STG rebuild from scratch succeeded)
