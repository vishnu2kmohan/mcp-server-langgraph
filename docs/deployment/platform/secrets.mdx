...
<Tabs>
  ...
  <Tab title="STG">
    ```bash
    # Set stg secrets
    langsmith secret set ANTHROPIC_API_KEY "stg-key" \
      --deployment my-agent-stg

    langsmith secret set LANGSMITH_PROJECT "my-agent-stg" \
      --deployment my-agent-stg
    ```
  </Tab>
...
<AccordionGroup>
  ...
  <Accordion title="Use Deployment-Specific Secrets">
    Never share secrets between dev/stg/prod:
