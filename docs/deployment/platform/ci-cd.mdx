...
<Info>
  **Automated Deployments**: Push to main → CI/CD tests → Auto-deploy to prod
</Info>
  ...
  deploy-stg:
    ...
    steps:
      ...
      - name: Deploy to STG
        ...
        run: |
          langgraph deploy stg-mcp-server-langgraph --tag stg

      - name: Smoke Test STG
        run: |
          langgraph deployment invoke stg-mcp-server-langgraph \
...
<Steps>
  ...
  <Step title="Create 'stg' Environment">
    - Name: `stg`
    - No approvals needed
...
deploy-stg:
  ...
  environment:
    name: stg
    url: https://stg-mcp-server-langgraph.langchain.com
  script:
    - curl -LsSf https://astral.sh/uv/install.sh | sh
...
<AccordionGroup>
  ...
  <Accordion title="Use STG Environment">
    ```yaml
    deploy-stg:
      # Deploy to stg first
      # Run smoke tests
    ...
    deploy-prod:
      needs: deploy-stg  # Only deploy to prod if stg succeeds
    ```
  </Accordion>
...
<Tabs>
  ...
  <Tab title="STG">
    **Trigger**: Every push to `main` branch
    ...
    > NOTE: Set the repository variable `ENABLE_STG_AUTODEPLOY` to `true` to enable automated stg deploys. Leaving it unset keeps CI from stalling on stg approvals.

    ```yaml
    deploy-stg:
      if: github.ref == 'refs/heads/main'
      run: langgraph deploy my-agent-stg --tag stg
    ```
  </Tab>
...
<Check>
  **CI/CD Ready!** Your LangGraph Platform deployments are now automated with testing, stg, and production workflows.
</Check>
