...
description: "GDPR Storage Backend Configuration"
icon: "rocket"
---
...
```mermaid
sequenceDiagram
    participant User as Data Subject<br/>(End User)
    participant API as GDPR API<br/>/api/v1/users/me/*
    participant Auth as Authentication<br/>(JWT Validation)
    participant Storage as Storage Backend<br/>(PostgreSQL/Redis)
    participant DB as Conversation DB<br/>(PostgreSQL)
    participant FGA as OpenFGA<br/>Authorization
    participant Logs as Log System<br/>(Cloud Logging)
    ...
    classDef apiStyle fill:#fdb462,stroke:#e67e22,stroke-width:2px
    classDef authStyle fill:#bebada,stroke:#8e44ad,stroke-width:2px
    classDef storageStyle fill:#b3de69,stroke:#7cb342,stroke-width:2px
    classDef dbStyle fill:#80b1d3,stroke:#3498db,stroke-width:2px
    classDef fgaStyle fill:#fb8072,stroke:#c0392b,stroke-width:2px
    classDef logStyle fill:#ffffb3,stroke:#f39c12,stroke-width:2px
    classDef auditStyle fill:#bc80bd,stroke:#8e44ad,stroke-width:2px
