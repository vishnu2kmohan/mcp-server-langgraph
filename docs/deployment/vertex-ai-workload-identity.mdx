...
flowchart TB
    subgraph Pod["GKE Pod (mcp-server-langgraph)"]
        K8sSA[Kubernetes Service Account<br/>mcp-stg/mcp-server-langgraph<br/>• Annotated with GCP SA]

        K8sSA -->|Workload Identity Binding| GCPSA[GCP Service Account<br/>vertex-ai-stg<br/>• roles/aiplatform.user<br/>• roles/aiplatform.developer<br/>• No keys required!]
...
flowchart TB
    ...
    subgraph GKE["GKE Cluster (stg-mcp-server-langgraph-gke)"]
        Pod[GKE Pod<br/>mcp-server-langgraph<br/>• No credential files<br/>• Uses SA token projection]
        K8sSA[Kubernetes Service Account<br/>mcp-stg/mcp-server-langgraph<br/>• Annotated with GCP SA<br/>• Token mounted as volume]
    ...
    subgraph IAM["GCP IAM"]
        GCPSA[GCP Service Account<br/>vertex-ai-stg@project.iam<br/>• roles/aiplatform.user<br/>• roles/aiplatform.developer<br/>• roles/logging.logWriter<br/>• roles/monitoring.metricWriter]
    end
...
- ✅ **STG infrastructure** deployed (run `setup-stg-infrastructure.sh` first)
...
2. ✅ Create `vertex-ai-stg` service account
...
gcloud iam service-accounts create vertex-ai-stg \
  --display-name="Vertex AI STG" \
  --description="Service account for Vertex AI access in stg" \
  --project=vishnu-sandbox-20250310
```
...
SA_EMAIL="vertex-ai-stg@${PROJECT_ID}.iam.gserviceaccount.com"
...
gcloud iam service-accounts add-iam-policy-binding \
  vertex-ai-stg@vishnu-sandbox-20250310.iam.gserviceaccount.com \
  --role="roles/iam.workloadIdentityUser" \
  --member="serviceAccount:vishnu-sandbox-20250310.svc.id.goog[mcp-stg/mcp-server-langgraph]"
```
#### Step 5: Annotate Kubernetes Service Account

```bash
kubectl annotate serviceaccount mcp-server-langgraph \
  -n stg-mcp-server-langgraph \
  iam.gke.io/gcp-service-account=vertex-ai-stg@vishnu-sandbox-20250310.iam.gserviceaccount.com \
  --overwrite
```
</Accordion>
...
The deployment is configured with the following environment variables in `deployments/overlays/stg-gke/deployment-patch.yaml`:
...
## Wait for rollout
kubectl rollout status deployment/mcp-server-langgraph -n stg-mcp-server-langgraph

## Verify pods are running
kubectl get pods -n stg-mcp-server-langgraph
```
...
## serviceAccount:vishnu-sandbox-20250310.svc.id.goog[mcp-stg/mcp-server-langgraph]
```
#### 2. Verify Kubernetes Annotation

```bash
## Check service account annotation
kubectl get serviceaccount mcp-server-langgraph -n stg-mcp-server-langgraph \
...
POD_NAME=$(kubectl get pods -n stg-mcp-server-langgraph -l app=mcp-server-langgraph -o jsonpath='{.items[0].metadata.name}')

## Check active service account
kubectl exec -it $POD_NAME -n stg-mcp-server-langgraph -- gcloud auth list
...
kubectl exec -it $POD_NAME -n stg-mcp-server-langgraph -- python3 << 'EOF'
...
<AccordionGroup>
  <Accordion title="Pod cannot authenticate" icon="user-lock">
    ...
    **Solutions**:
    ...
    2. Check service account annotation:
       ```bash
       kubectl describe sa mcp-server-langgraph -n stg-mcp-server-langgraph | grep Annotations
       ```
    ...
    **Solutions**:
    1. Verify service account has required roles:
       ...
       gcloud projects get-iam-policy vishnu-sandbox-20250310 \
         ...
         --filter="bindings.members:serviceAccount:vertex-ai-stg@vishnu-sandbox-20250310.iam.gserviceaccount.com"
       ```
    2. Grant missing permissions:
       ```bash
       # Vertex AI User
       gcloud projects add-iam-policy-binding vishnu-sandbox-20250310 \
         --member="serviceAccount:vertex-ai-stg@vishnu-sandbox-20250310.iam.gserviceaccount.com" \
         --role="roles/aiplatform.user"
  ...
  <Accordion title="Workload Identity not working" icon="link-slash">
    **Error**: Pod uses default compute service account instead of vertex-ai-stg
    ...
    **Solutions**:
    ...
    2. Check if annotation was applied before pod creation:
       ...
       kubectl delete pod $POD_NAME -n stg-mcp-server-langgraph
       # Pod will be recreated by deployment
       ```
    3. Verify namespace has Workload Identity enabled:
       ```bash
       kubectl get namespace stg-mcp-server-langgraph -o yaml | grep workload-identity
       ```
  </Accordion>
</AccordionGroup>
...
The `vertex-ai-stg` service account has been granted these roles:
...
<Steps>
  ...
  <Step title="Verify Migration">
    ...
    # Test Vertex AI
    kubectl logs -n stg-mcp-server-langgraph $POD_NAME | grep "Vertex AI"
    ```
  </Step>
</Steps>
...
<CardGroup cols={2}>
  ...
  <Card title="GKE Preview Deployment" icon="kubernetes" href="/deployment/kubernetes/gke-stg">
    Complete GKE stg deployment guide
  </Card>
...
<Check>
**Vertex AI with Workload Identity Configured!** Your stg environment now uses keyless authentication for secure, scalable AI access.
</Check>
