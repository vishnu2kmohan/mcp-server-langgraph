...
```
┌────────────────────────────────────────────────────────┐
│  GKE Pod (mcp-server-langgraph)                        │
│  ┌──────────────────────────────────────────────────┐ │
│  │  Kubernetes Service Account                      │ │
│  │  • mcp-staging/mcp-server-langgraph             │ │
│  │  • Annotated with GCP SA                         │ │
│  └──────────────────┬───────────────────────────────┘ │
│                     │ Workload Identity Binding        │
│                     ▼                                   │
│  ┌──────────────────────────────────────────────────┐ │
│  │  GCP Service Account (vertex-ai-staging)         │ │
...
The deployment is configured with the following environment variables in `deployments/overlays/staging-gke/deployment-patch.yaml`:
...
kubectl get serviceaccount mcp-server-langgraph -n mcp-staging \
  ...
  <Accordion title="Pod cannot authenticate" icon="user-lock">
    ...
    **Solutions**:
    ...
    2. Check service account annotation:
       ```bash
       kubectl describe sa mcp-server-langgraph -n mcp-staging | grep Annotations
       ```
  ...
  <Accordion title="Workload Identity not working" icon="link-slash">
    ...
    **Solutions**:
    ...
    2. Check if annotation was applied before pod creation:
       ...
       kubectl delete pod $POD_NAME -n mcp-staging
       # Pod will be recreated by deployment
       ```

    3. Verify namespace has Workload Identity enabled:
       ```bash
       kubectl get namespace mcp-staging -o yaml | grep workload-identity
       ```
  </Accordion>
</AccordionGroup>
...
<Steps>
  ...
  <Step title="Verify Migration">
    ...
    # Test Vertex AI
    kubectl logs -n mcp-staging $POD_NAME | grep "Vertex AI"
    ```
  </Step>
</Steps>
