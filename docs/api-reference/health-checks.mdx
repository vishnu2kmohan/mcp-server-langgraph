...
spec:
  template:
    spec:
      containers:
      - image: gcr.io/project/langgraph-agent:latest
        ports:
        - containerPort: 8000
...
<AccordionGroup>
  <Accordion title="Service Shows Unhealthy" icon="triangle-exclamation">
    ...
    kubectl logs -l app=langgraph-agent --tail=100 | grep -i error
  ...
  <Accordion title="Pods Not Ready" icon="hourglass">
    ...
    kubectl exec -it <pod-name> -n langgraph-agent -- \
  ...
  <Accordion title="Pods Restarting" icon="rotate">
    ...
    kubectl get pod <pod-name> -n langgraph-agent \
  ...
  <Accordion title="Slow Health Checks" icon="clock">
    ...
    kubectl patch deployment langgraph-agent -n langgraph-agent \
      --type json -p='[{
        "op": "replace",
