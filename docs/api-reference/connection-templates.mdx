...
<Info>
Connection templates are read-only. To create an actual connection from a template, call `POST /api/v1/connections` with the template's pre-filled values.
</Info>
...
flowchart LR
    ...
    classDef templateStyle fill:#8dd3c7,stroke:#2a9d8f,stroke-width:2px,color:#333
...
- `POST /api/v1/connections` - Create and manage actual connections
