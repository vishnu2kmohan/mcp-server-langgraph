...
icon: 'wrench'
seoTitle: "MCP Tools - MCP Protocol API"
seoDescription: "MCP Protocol reference: Available tools and their usage in the Model Context Protocol"
...
flowchart TB
    ...
    subgraph Discovery["Progressive Discovery"]
        Query[Search Query]
    ...
    subgraph Output["Response"]
        Minimal[Minimal: name + desc]
        Standard[Standard: + params]
        Full[Full: + schema + examples]
    end
    ...
    Register --> Code
    Query --> Filter
    Filter -->|Yes| CatFilter
    Filter -->|No| TextFilter
    CatFilter --> Match
    TextFilter --> Match
    Match --> Minimal
    Match --> Standard
    ...
    classDef regStyle fill:#8dd3c7,stroke:#2a9d8f,stroke-width:2px,color:#333
    classDef catStyle fill:#fdb462,stroke:#e67e22,stroke-width:2px,color:#333
    classDef discStyle fill:#bebada,stroke:#7e5eb0,stroke-width:2px,color:#333
    classDef outStyle fill:#b3de69,stroke:#7cb342,stroke-width:2px,color:#333

    class Def,Schema,Register regStyle
    class Calc,Search,FS,Code catStyle
    class Query,Filter,TextFilter,CatFilter,Match discStyle
    class Minimal,Standard,Full outStyle
```
