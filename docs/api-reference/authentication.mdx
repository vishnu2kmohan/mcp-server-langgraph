...
<Warning>
**RFC 9700 Compliance Notice**: The Resource Owner Password Credentials (ROPC) grant type (`grant_type: password`) is **deprecated** per [RFC 9700 (OAuth 2.0 Security Best Practice)](https://datatracker.ietf.org/doc/rfc9700/). Use **OAuth2 Authorization Code + PKCE** flow via `GET /auth/login` instead. ROPC endpoints will be removed in v2.0.

See [ADR-0071](/docs-internal/ADR-0071-OAUTH2-PKCE-MIGRATION.md) for migration details.
</Warning>
...
<ParamField path="grant_type" type="string" default="password">
  OAuth2 grant type. Options: `password` (⚠️ DEPRECATED per RFC 9700), `refresh_token`

  **Recommended**: Use `GET /auth/login` for OAuth2 Authorization Code + PKCE flow instead.
</ParamField>
