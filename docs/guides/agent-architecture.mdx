...
flowchart TB
    ...
    subgraph Agent["Agent Node"]
        LLM[LLM Invocation<br/>+ Message History]
        Response[AI Response]
    end

    subgraph Routing["Conditional Routing"]
        Check{Tool Calls<br/>in Response?}
    end

    subgraph Tools["Tool Node"]
        Exec[Execute Tools<br/>in Parallel]
        Results[Tool Results<br/>as Messages]
    end

    subgraph End["End"]
        Final[Final Response<br/>to User]
    end
    ...
    classDef entryStyle fill:#8dd3c7,stroke:#2a9d8f,stroke-width:2px,color:#333
    classDef agentStyle fill:#b3de69,stroke:#7cb342,stroke-width:2px,color:#333
    classDef routingStyle fill:#ffffb3,stroke:#f1c40f,stroke-width:2px,color:#333
    classDef toolsStyle fill:#fdb462,stroke:#e67e22,stroke-width:2px,color:#333
    classDef endStyle fill:#bebada,stroke:#7e5eb0,stroke-width:2px,color:#333

    class Start entryStyle
    class LLM,Response agentStyle
    class Check routingStyle
    class Exec,Results toolsStyle
    class Final endStyle
```
...
flowchart LR
    subgraph Dev["Development"]
        Memory[MemorySaver<br/>In-Memory]
        MemProps["Fast startup<br/>No persistence<br/>Single process"]
    end

    subgraph Prod["Production"]
        Postgres[PostgresSaver<br/>PostgreSQL]
        PgProps["Persistent state<br/>Multi-replica<br/>Crash recovery"]
    end

    subgraph Redis["Alternative"]
        RedisCP[RedisSaver<br/>Redis Cluster]
        RedisProps["Fast reads<br/>Distributed<br/>TTL support"]
    end
    ...
    classDef prodStyle fill:#80b1d3,stroke:#3498db,stroke-width:2px,color:#333
    classDef redisStyle fill:#fdb462,stroke:#e67e22,stroke-width:2px,color:#333

    class Memory,MemProps devStyle
    class Postgres,PgProps prodStyle
    class RedisCP,RedisProps redisStyle
```
...
- **Project Guides**:
  - [Pydantic AI Integration](/architecture/adr-0005-pydantic-ai-integration)
  - [Multi-LLM Setup Guide](/guides/multi-llm-setup)
  - [Testing Guide](/advanced/testing)
  - [Development Setup](/advanced/development-setup)
