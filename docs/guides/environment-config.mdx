...
In production/staging environments, push subscriptions and AI feedback are stored in PostgreSQL. Configure the database URL:

```properties
## Database for persistent stores (production/staging)
...
- **Production/Staging**: PostgreSQL stores (requires `DATABASE_URL`)
