...
<Steps>
  <Step title="Sign Up">
    ...
    4. Create project: "langgraph-agent"
  </Step>
  ...
  authentication:
    universalAuth:
      credentialsRef:
        ...
        secretNamespace: langgraph-agent
  ...
  template:
    spec:
      ...
      - name: agent
        envFrom:
        - secretRef:
            name: langgraph-agent-secrets  # Auto-synced from Infisical
```
...
<CardGroup cols={2}>
  ...
  <Card title="Configuration" icon="gear" href="/getting-started/installation#configuration">
