...
description: 'Configure OpenAI GPT models including GPT-4o, GPT-4 Turbo, and GPT-3.5'
icon: 'robot'
---

### Overview

OpenAI provides industry-leading language models including GPT-4o (multimodal), GPT-4 Turbo, and GPT-3.5 Turbo. This guide covers setup, configuration, and best practices for using OpenAI models with the MCP Server.

<Info>
GPT-4o combines GPT-4-level intelligence with multimodal capabilities (text, vision, audio) at half the price of GPT-4 Turbo.
</Info>
...
|-------|---------------|----------|-------------------------|
| gpt-4o | 128K tokens | Best overall, multimodal | $2.50 input / $10.00 output |
| gpt-4o-mini | 128K tokens | Fast, affordable | $0.15 input / $0.60 output |
| gpt-4-turbo | 128K tokens | Advanced reasoning | $10.00 input / $30.00 output |
| gpt-4 | 8K tokens | High intelligence | $30.00 input / $60.00 output |
| gpt-3.5-turbo | 16K tokens | Fast, cost-effective | $0.50 input / $1.50 output |

**Recommended**: `gpt-4o` for production (best performance/cost ratio)
...
def select_openai_model(task_type: str, complexity: str) -> str:
    ...
    # Multimodal or complex reasoning
        ...
        return "gpt-4o"  # Best for multimodal
...
async def call_with_fallback(query: str):
    try:
        ...
        fallback_llm = LLMFactory(provider="anthropic", model_name="claude-3-5-sonnet-20241022")
...
async def send_message(
    ...
    # Estimate cost
    input_tokens = count_tokens(request.query, model="gpt-4o")
...
async def call_openai_with_metrics(query: str):
    ...
    try:
        ...
        openai_latency.labels(model="gpt-4o").observe(duration)
```
### Troubleshooting
...
<AccordionGroup>
  ...
  <Accordion title="Context length exceeded">
    ...
    **Solutions**:
    ...
    truncated = truncate_to_tokens(long_text, max_input_tokens, model="gpt-4o")

    # Summarize first
    summary_llm = LLMFactory(provider="openai", model_name="gpt-4o-mini")
...
<Check>
**OpenAI Ready**: Leverage GPT-4o and other OpenAI models in your MCP Server!
</Check>
