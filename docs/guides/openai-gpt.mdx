...
|-------|---------------|----------|-------------------------|
| gpt-5 | 128K tokens | Best all-around, latest flagship | $2.50 input / $10.00 output |
| gpt-5-pro | 128K tokens | Most capable, advanced reasoning | $10.00 input / $30.00 output |
| gpt-5-mini | 128K tokens | Fast and cost-effective | $0.15 input / $0.60 output |
| gpt-5-nano | 128K tokens | Smallest, fastest, ultra low-cost | $0.10 input / $0.40 output |

**Recommended**: `gpt-5` for production (best performance/cost ratio)
...
def select_openai_model(task_type: str, complexity: str) -> str:
    ...
    if task_type == "chat" and complexity == "simple":
        return "gpt-5-nano"  # Smallest, fastest, ultra low-cost
    ...
    # Multimodal or complex reasoning
        ...
        return "gpt-5"  # Best all-around flagship

    # Maximum intelligence and advanced reasoning
    elif complexity == "expert":
        return "gpt-5-pro"  # Most capable
...
def get_optimal_model(complexity: str) -> str:
    if complexity == "simple":
        return "gpt-5-nano"  # $0.10/$0.40 per 1M tokens (smallest, fastest)
    elif complexity == "low":
        return "gpt-5-mini"  # $0.15/$0.60 per 1M tokens (cost-effective)
    elif complexity == "medium":
        return "gpt-5"  # $2.50/$10.00 per 1M tokens (flagship)
    else:
        return "gpt-5-pro"  # $10/$30 per 1M tokens (most capable)
...
async def send_message(
    ...
    # Estimate cost
    input_tokens = count_tokens(request.query, model="gpt-5")
...
async def call_openai_with_metrics(query: str):
    ...
    try:
        ...
        openai_latency.labels(model="gpt-5").observe(duration)
```
### Troubleshooting
...
<AccordionGroup>
  ...
  <Accordion title="Context length exceeded">
    ...
    **Solutions**:
    ...
    truncated = truncate_to_tokens(long_text, max_input_tokens, model="gpt-5")
...
<AccordionGroup>
  ...
  <Accordion title="Cost Management" icon="dollar-sign">
    ...
    - Set budget alerts
    - Use cheaper models for simple tasks (gpt-5-nano, gpt-5-mini)
    - Implement token limits
    - Monitor and optimize prompt efficiency
    - Use gpt-5-mini or gpt-5-nano for development/testing
  </Accordion>
