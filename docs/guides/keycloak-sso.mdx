  ...
  <Step title="Deploy Keycloak">
    <Tabs>
      ...
      <Tab title="Kubernetes">
        ...
        ```bash
        kubectl port-forward svc/keycloak 8080:80 -n langgraph-agent
        ```
      </Tab>
...
const authUrl = `https://sso.yourdomain.com/realms/langgraph-agent/protocol/openid-connect/auth?
...
const tokens = await fetch('https://sso.yourdomain.com/realms/langgraph-agent/protocol/openid-connect/token', {
  method: 'POST',
...
kubectl exec -n langgraph-agent \
  $(kubectl get pod -n langgraph-agent -l app=postgresql -o jsonpath='{.items[0].metadata.name}') \
  -- pg_dump -U keycloak keycloak > keycloak-backup.sql

# Restore
kubectl exec -i -n langgraph-agent \
  $(kubectl get pod -n langgraph-agent -l app=postgresql -o jsonpath='{.items[0].metadata.name}') \
