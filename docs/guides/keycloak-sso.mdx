...
icon: 'key'
seoTitle: "Keycloak SSO Integration - Setup Guide"
seoDescription: "Step-by-step guide: Enterprise authentication with Keycloak OpenID Connect"
...
flowchart LR
    subgraph AuthCode["Authorization Code Flow"]
        A1[User] -->|1. Login| A2[Keycloak]
        A2 -->|2. Code| A3[Callback]
        A3 -->|3. Exchange| A4[Token]
    end

    subgraph Direct["Direct Grant (Testing)"]
        D1[App] -->|Username/Password| D2[Keycloak]
        D2 -->|Token| D3[Response]
    end

    subgraph Refresh["Token Refresh"]
        R1[Refresh Token] -->|Request| R2[Keycloak]
        R2 -->|New Tokens| R3[Rotated]
    end

    %% ColorBrewer2 Set3 palette
    classDef authStyle fill:#8dd3c7,stroke:#2a9d8f,stroke-width:2px,color:#333
    classDef directStyle fill:#fb8072,stroke:#e74c3c,stroke-width:2px,color:#333
    classDef refreshStyle fill:#b3de69,stroke:#7cb342,stroke-width:2px,color:#333

    class A1,A2,A3,A4 authStyle
    class D1,D2,D3 directStyle
    class R1,R2,R3 refreshStyle
```
...
```mermaid
flowchart TB
    subgraph Keycloak["Keycloak Identity"]
        User[User: alice]
    ...
    subgraph Mapping["Role Mapping Engine"]
        Simple[Simple Mappings<br/>admin → admin]
        Regex[Regex Groups<br/>/admins → admin]
        Cond[Conditional<br/>email_verified → member]
        Hier[Hierarchies<br/>admin → member → viewer]
    end

    subgraph OpenFGA["OpenFGA Tuples"]
        T1["user:alice#admin"]
        T2["user:alice#member"]
        T3["user:alice#viewer"]
    end
    ...
    %% ColorBrewer2 Set3 palette
    classDef kcStyle fill:#8dd3c7,stroke:#2a9d8f,stroke-width:2px,color:#333
    classDef mapStyle fill:#fdb462,stroke:#e67e22,stroke-width:2px,color:#333
    classDef fgaStyle fill:#b3de69,stroke:#7cb342,stroke-width:2px,color:#333

    class User,Roles,Groups kcStyle
    class Simple,Regex,Cond,Hier mapStyle
    class T1,T2,T3 fgaStyle
```
...
```bash
## Keycloak health (with KC_HTTP_RELATIVE_PATH=/authn for gateway routing)
curl https://sso.yourdomain.com/authn/health

## Readiness
curl https://sso.yourdomain.com/authn/health/ready

## Liveness
curl https://sso.yourdomain.com/authn/health/live

## Note: Health check paths include /authn prefix when KC_HTTP_RELATIVE_PATH=/authn is configured
## For development (no gateway), use /health, /health/ready, /health/live
```
#### Metrics
...
<AccordionGroup>
  <Accordion title="Connection refused">
    ...
    # Test connectivity (with KC_HTTP_RELATIVE_PATH=/authn)
    curl http://keycloak:8080/authn/health/ready
    # Or without relative path:
  ...
  <Accordion title="Healthcheck returns 404 Not Found">
    **Root Cause**: When `KC_HTTP_RELATIVE_PATH=/authn` is configured, ALL endpoints
    are prefixed with `/authn`, including health endpoints on the management port (9000).

    **Problem**: Using `/health/ready` returns 404 when Keycloak is configured with a relative path.

    **Solution**: Always use `/authn/health/ready` when `KC_HTTP_RELATIVE_PATH=/authn` is set.

    ```bash
    # WRONG (returns 404 when KC_HTTP_RELATIVE_PATH=/authn)
    curl http://localhost:9000/health/ready

    # CORRECT (works with KC_HTTP_RELATIVE_PATH=/authn)
    curl http://localhost:9000/authn/health/ready
    ...
    **Docker healthcheck pattern** (bash TCP socket - no curl available in Keycloak 26+):
    ```bash
    # Phase 1: Health check on management port
    { printf 'HEAD /authn/health/ready HTTP/1.0\r\nHost: localhost\r\n\r\n' >&0; \
      grep -q 'HTTP/1.0 200'; } 0<>/dev/tcp/localhost/9000

    # Phase 2: OIDC discovery validation on HTTP port
    { printf 'GET /authn/realms/default/.well-known/openid-configuration HTTP/1.0\r\nHost: localhost\r\n\r\n' >&0; \
      grep -q '"issuer"'; } 0<>/dev/tcp/localhost/8080
    ```

    <Warning>
    **Important**: Keycloak 26+ images do NOT include `curl` or `wget` (security hardening).
    Use the bash `/dev/tcp` socket pattern for healthchecks.
    </Warning>
  </Accordion>

  <Accordion title="OIDC discovery endpoint not ready">
    Dependent services (OpenFGA, traefik-forward-auth) may fail to start if Keycloak's
    OIDC discovery endpoint isn't ready.

    **Symptoms**:
    - OpenFGA fails with "failed to fetch OIDC configuration"
    - traefik-forward-auth returns 500 errors
    - Services timeout waiting for Keycloak

    **Solution**: Use a two-phase healthcheck that validates OIDC discovery:
    ...
    This ensures dependent services only start when OIDC is fully functional.
  </Accordion>
</AccordionGroup>
