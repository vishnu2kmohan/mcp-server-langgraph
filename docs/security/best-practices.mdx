...
flowchart LR
    ...
    Auth --> Network --> Data --> Ops

    %% ColorBrewer2 Set3 palette - security layers
    classDef authStyle fill:#8dd3c7,stroke:#2a9d8f,stroke-width:2px,color:#333
    classDef networkStyle fill:#bebada,stroke:#7e5eb0,stroke-width:2px,color:#333
    classDef dataStyle fill:#ffffb3,stroke:#f1c40f,stroke-width:2px,color:#333
    classDef opsStyle fill:#b3de69,stroke:#7cb342,stroke-width:2px,color:#333

    class MFA,Strong,JWT authStyle
    class TLS,Firewall,CORS networkStyle
    class Encrypt,Secrets,Backup dataStyle
    class Audit,Monitor,Incident opsStyle
```
