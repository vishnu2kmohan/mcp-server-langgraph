---
title: GKE STG Security Checklist
description: 'Comprehensive security checklist to verify that your GKE stg environment follows security best practices'
icon: 'clipboard-check'
seoTitle: "GKE STG Security Checklist - Security Guide"
seoDescription: "Security best practices: Comprehensive security checklist to verify that your GKE stg environment follows security best practices"
keywords: ["security", "enterprise security", "compliance", "best practices"]
contentType: "explanation"
---

This checklist helps verify that your GKE stg environment follows security best practices.

### Network Security

#### VPC Isolation
- [ ] STG uses a separate VPC from production
...
gcloud compute networks subnets describe stg-gke-subnet \
  --region=us-central1 \
  --format=yaml
```
...
kubectl describe networkpolicy default-deny-ingress -n stg-mcp-server-langgraph
kubectl describe networkpolicy allow-egress -n stg-mcp-server-langgraph
```
...
**Verification:**
```bash
## List firewall rules for stg VPC
gcloud compute firewall-rules list --filter="network:stg-vpc"
```
...
gcloud iam service-accounts keys list \
  --iam-account=mcp-stg-sa@vishnu-sandbox-20250310.iam.gserviceaccount.com
```
#### Workload Identity
...
## Verify permissions
gcloud secrets get-iam-policy stg-jwt-secret
```
...
kubectl get externalsecret mcp-stg-secrets -n stg-mcp-server-langgraph

## Verify secrets are synced
kubectl get secret mcp-stg-secrets -n stg-mcp-server-langgraph
```
### Container Security
...
```bash
## Check image source
kubectl get deployment stg-mcp-server-langgraph -n stg-mcp-server-langgraph \
...
kubectl get pod -n stg-mcp-server-langgraph -o json | \
  jq '.items[].spec.securityContext'

## Check for privileged containers
kubectl get pods -n stg-mcp-server-langgraph -o json | \
...
gcloud sql instances describe mcp-stg-postgres \
...
gcloud redis instances describe mcp-stg-redis \
  --region=us-central1
...
kubectl logs -n stg-mcp-server-langgraph -l app=otel-collector
```
...
kubectl get pods -n stg-mcp-server-langgraph --show-labels
```
#### Resource Limits
...
## Check ResourceQuota
kubectl get resourcequota -n stg-mcp-server-langgraph

## Check LimitRange
kubectl get limitrange -n stg-mcp-server-langgraph
```
...
## List backups
gcloud sql backups list --instance=mcp-stg-postgres
```
...
**Verification:**
- Review `.github/workflows/deploy-stg-gke.yaml`
- Check GitHub repository settings → Environments → stg
...
gcloud artifacts docker images scan \
  us-central1-docker.pkg.dev/vishnu-sandbox-20250310/mcp-stg/agent:stg-latest
```
#### Security Auditing
...
- **&lt;70% complete**: Poor - Not ready for production-like stg
