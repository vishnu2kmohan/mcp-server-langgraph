...
<AccordionGroup>
  ...
  <Accordion title="Verify Network Policies">
    ```bash
    kubectl get networkpolicies -n prod-mcp-server-langgraph
    ```
...
<Steps>
  ...
  <Step title="Containment">
    ...
    kubectl label pod SUSPICIOUS_POD quarantine=true -n prod-mcp-server-langgraph
  ...
  <Step title="Investigation">
    ```bash
    # Export logs
    kubectl logs POD_NAME -n prod-mcp-server-langgraph --all-containers > incident-logs.txt
