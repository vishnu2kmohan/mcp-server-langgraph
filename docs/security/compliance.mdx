...
icon: 'scale-balanced'
seoTitle: "Compliance & Regulations - Security Guide"
seoDescription: "Security best practices: GDPR, SOC 2, HIPAA, and other compliance requirements"
...
flowchart TD
    ...
    subgraph Controls["Technical Controls"]
        ...
        Privacy["Data Privacy<br/>& Consent"]
    end

    subgraph Outcomes["Compliance Outcomes"]
        Evidence["Evidence<br/>Collection"]
        Reports["Compliance<br/>Reports"]
        Alerts["Violation<br/>Alerts"]
    end
    ...
    classDef frameworkStyle fill:#8dd3c7,stroke:#2a9d8f,stroke-width:2px,color:#333
    classDef controlStyle fill:#bebada,stroke:#7e5eb0,stroke-width:2px,color:#333
    classDef outcomeStyle fill:#b3de69,stroke:#7cb342,stroke-width:2px,color:#333

    class GDPR,SOC2,HIPAA,ISO frameworkStyle
    class Auth,Encrypt,Audit,Privacy controlStyle
    class Evidence,Reports,Alerts outcomeStyle
```

MCP Server with LangGraph provides **technical controls and architecture** to support compliance with data protection regulations and industry standards. This guide covers implementing controls for [GDPR](https://gdpr-info.eu/), [SOC 2](https://www.aicpa.org/soc4so), [HIPAA](https://www.hhs.gov/hipaa/), and other compliance frameworks.

<Warning>
**Important Compliance Disclaimer:**

This codebase provides **technical controls** that support compliance requirements. However:
...
See [Sources & References](/reference/sources) for links to official compliance documentation.
</Warning>

### Compliance Frameworks

<CardGroup cols={2}>
  <Card title="GDPR" icon="shield-halved" href="https://gdpr-info.eu/">
  ...
  <Card title="SOC 2" icon="building-shield" href="https://www.aicpa.org/soc4so">
    **[Service Organization Control 2](https://www.aicpa.org/soc4so)**
  ...
  <Card title="HIPAA" icon="hospital" href="https://www.hhs.gov/hipaa/">
    **[Health Insurance Portability](https://www.hhs.gov/hipaa/)**
  ...
  <Card title="ISO 27001" icon="certificate" href="https://www.iso.org/standard/27001">
    **[Information Security Management](https://www.iso.org/standard/27001)**
...
flowchart TB
    ...
    subgraph DSR["Data Subject Requests (DSR)"]
        AccessReq["Access Request<br/>(Art. 15)"]
        RectifyReq["Rectification Request<br/>(Art. 16)"]
        EraseReq["Erasure Request<br/>(Art. 17)"]
        PortReq["Portability Request<br/>(Art. 20)"]
    end

    subgraph Classification["Data Classification & PII Identification"]
        Scanner["PII Scanner"]
        Classifier["Data Classifier"]
        PIITypes["PII Types:<br/>• Email<br/>• Name<br/>• IP Address<br/>• User ID"]
    end

    subgraph Consent["Consent Management"]
        ConsentUI["Consent UI"]
        ConsentStore["Consent Records"]
        ConsentLog["Consent Audit Log"]
        ConsentMeta["Metadata:<br/>• Timestamp<br/>• IP Address<br/>• User Agent"]
    end

    subgraph Retention["Data Retention Policies"]
        RetPolicy["Retention Policy<br/>(365 days)"]
        RetScheduler["Cleanup Scheduler"]
        RetAction["Auto-Delete<br/>Old Data"]
    end

    subgraph Erasure["Right to Erasure Flow"]
        EraseValidate["Validate Request"]
        EraseProcess["Delete User Data:<br/>• Conversations<br/>• Sessions<br/>• Preferences"]
        EraseAuth["Delete AuthZ Tuples"]
        EraseAnonymize["Anonymize Logs"]
        EraseConfirm["Confirm Deletion"]
    end

    subgraph Audit["Audit Logging"]
        AuditCapture["Capture Events:<br/>• Access<br/>• Export<br/>• Deletion<br/>• Consent Changes"]
        AuditStore["Tamper-Proof Store"]
        AuditReview["Regular Review"]
        AuditAlert["Anomaly Alerts"]
    end
    ...
    %% DSR Flow
    AccessReq --> Scanner
    AccessReq --> AuditCapture
    RectifyReq --> EraseValidate
    RectifyReq --> AuditCapture
    EraseReq --> Erasure
    PortReq --> Scanner
    PortReq --> AuditCapture

    %% Classification Flow
    Scanner --> Classifier
    Classifier --> PIITypes
    PIITypes --> EncryptedDB

    %% Consent Flow
    ConsentUI --> ConsentStore
    ConsentStore --> ConsentMeta
    ConsentMeta --> ConsentLog
    ConsentLog --> AuditCapture

    %% Retention Flow
    RetPolicy --> RetScheduler
    RetScheduler --> RetAction
    RetAction --> EncryptedDB
    RetAction --> AuditCapture

    %% Erasure Flow
    EraseValidate --> EraseProcess
    EraseProcess --> EraseAuth
    EraseAuth --> EraseAnonymize
    EraseAnonymize --> EraseConfirm
    EraseConfirm --> AuditCapture

    %% Audit Flow
    AuditCapture --> AuditStore
    AuditStore --> AuditReview
    AuditReview --> AuditAlert

    %% Storage connections
    EncryptedDB --> BackupStore
    ConsentStore --> EncryptedDB

    %% Apply ColorBrewer Set3 palette
    ...
    classDef auditStyle fill:#b3de69,stroke:#7cb342,stroke-width:2px,color:#333
    classDef consentStyle fill:#fb8072,stroke:#e74c3c,stroke-width:2px,color:#333
    classDef retentionStyle fill:#bebada,stroke:#7e5eb0,stroke-width:2px,color:#333
    classDef erasureStyle fill:#ffd92f,stroke:#f39c12,stroke-width:2px,color:#333
    classDef classifyStyle fill:#ffffb3,stroke:#f1c40f,stroke-width:2px,color:#333

    %% Apply styles to nodes
    class AccessReq,RectifyReq,EraseReq,PortReq requestStyle
    class Scanner,Classifier,PIITypes classifyStyle
    class ConsentUI,ConsentStore,ConsentLog,ConsentMeta consentStyle
    class RetPolicy,RetScheduler,RetAction retentionStyle
    class EraseValidate,EraseProcess,EraseAuth,EraseAnonymize,EraseConfirm erasureStyle
    class AuditCapture,AuditStore,AuditReview,AuditAlert auditStyle
    class EncryptedDB,BackupStore storageStyle
```
