...
<AccordionGroup>
  <Accordion title="✅ Keycloak Setup" icon="key">
    ...
    **Verification**:
    ...
    kubectl exec -it keycloak-0 -n langgraph-agent -- \
  ...
  <Accordion title="✅ JWT Configuration" icon="file-shield">
    ...
    **Verification**:
    ...
    kubectl get secret langgraph-agent-secrets -n langgraph-agent \
  ...
  <Accordion title="✅ OpenFGA Authorization" icon="shield-halved">
    ...
    **Verification**:
    ...
    # Check TLS
    kubectl get svc openfga -n langgraph-agent -o yaml | grep tls
    ```
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="✅ Logging & Monitoring" icon="file-lines">
    ...
    **Verification**:
    ...
    kubectl logs deployment/langgraph-agent -n langgraph-agent | \
      grep -i "password\|secret\|token"

    # Verify structured logging
    kubectl logs deployment/langgraph-agent -n langgraph-agent | jq .
    ```
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="✅ Container Images" icon="box">
    ...
    **Verification**:
    ```bash
    # Scan image
    trivy image langgraph-agent:latest

    # Check for root user
    docker inspect langgraph-agent:latest | grep -i "user"

    # Verify image signature
    cosign verify langgraph-agent:latest
    ```
  </Accordion>
  ...
  <Accordion title="✅ Runtime Security" icon="shield-halved">
    ...
    **Verification**:
    ...
    kubectl get pod langgraph-agent-xxx -n langgraph-agent \
      -o yaml | grep -A 10 securityContext

    # Verify Pod Security Standards
    kubectl label namespace langgraph-agent \
...
<AccordionGroup>
  <Accordion title="✅ Encryption" icon="lock">
    ...
    **Verification**:
    ...
    kubectl exec postgres-0 -n langgraph-agent -- \
...
<AccordionGroup>
  <Accordion title="✅ RBAC" icon="users-gear">
    ...
    **Verification**:
    ...
    kubectl auth can-i --list --as=system:serviceaccount:langgraph-agent:langgraph-agent
  ...
  <Accordion title="✅ Pod Security" icon="cube">
    ...
    **Verification**:
    ...
    kubectl get ns langgraph-agent -o yaml | grep pod-security
...
<CardGroup cols={2}>
  ...
  <Card title="Penetration Testing" icon="user-secret" href="/security/pen-testing">
