...
flowchart TD
    ...
    %% STG Deployment
    PushDev[Push to develop] --> DSG[deploy-stg-gke.yaml]
    ManualS[Manual trigger] --> DSG

    DSG --> AuthS[GKE auth]
    AuthS --> HelmS[Helm deploy stg]
    HelmS --> SmokeS[Smoke tests]

    %% Production Deployment
    Release[Release published] --> DPG[deploy-prod-gke.yaml]
...
- deploy-stg-gke.yaml (push to develop OR manual)
- deploy-prod-gke.yaml (release OR manual with approval)
