...
#### Test Image (final-test target)

The test image is built from `docker/Dockerfile` with `--target final-test`:
- Python 3.12
- Test dependencies only (`--extra dev`)
- Environment pre-configured (`TESTING=true`)
