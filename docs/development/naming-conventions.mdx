...
| STG | `stg` | Pre-production testing, integration validation |
...
|----------|--------|---------|
| GKE Cluster | `-gke` | `stg-mcp-server-langgraph-gke` |
| EKS Cluster | `-eks` | `stg-mcp-server-langgraph-eks` |
| AKS Cluster | `-aks` | `stg-mcp-server-langgraph-aks` |
| Namespace | *(none)* | `stg-mcp-server-langgraph` |
| Deployment | *(none)* | `stg-mcp-server-langgraph` |
| Service | *(none)* | `stg-mcp-server-langgraph` |
...
STG:          stg-mcp-server-langgraph-gke
...
|----------|---------|---------|
| VPC | `{short_prefix}-vpc` | `stg-mcp-slg-vpc` |
| Subnet | `{short_prefix}-{purpose}-subnet` | `stg-mcp-slg-nodes-us-central1` |
| Cloud SQL | `{short_prefix}-postgres` | `stg-mcp-slg-postgres` |
| Memorystore | `{short_prefix}-redis` | `stg-mcp-slg-redis` |
| Service Account | `{short_prefix}-{role}-sa` | `stg-mcp-slg-app-sa` |
| Artifact Registry | `mcp-{env}` | `mcp-stg`, `mcp-prod` |

**Note**: Short prefix = `{env}-mcp-slg` (e.g., `stg-mcp-slg`, `prod-mcp-slg`, `dev-mcp-slg`)
...
STG:          stg-mcp-server-langgraph-eks
...
| Resource | Pattern | Example |
|----------|---------|---------|
| VPC | `{name_prefix}-vpc` | `stg-mcp-server-langgraph-vpc` |
| Subnet | `{name_prefix}-{az}-{type}` | `stg-mcp-server-langgraph-us-east-1a-private` |
| RDS Instance | `{name_prefix}-db` | `stg-mcp-server-langgraph-db` |
| ElastiCache | `{name_prefix}-redis` | `stg-mcp-server-langgraph-redis` |
| IAM Role | `{cluster_name}-{purpose}` | `stg-mcp-server-langgraph-eks-cluster-role` |
...
STG:          stg-mcp-server-langgraph-aks
...
| Resource | Pattern | Example |
|----------|---------|---------|
| Resource Group | `{name_prefix}-rg` | `stg-mcp-server-langgraph-rg` |
| Virtual Network | `{name_prefix}-vnet` | `stg-mcp-server-langgraph-vnet` |
| PostgreSQL | `{name_prefix}-db` | `stg-mcp-server-langgraph-db` |
| Redis Cache | `{name_prefix}-redis` | `stg-mcp-server-langgraph-redis` |
...
|-----------|-------------|----------------|
| `mcp-server-langgraph` (Deployment) | stg | `stg-mcp-server-langgraph` |
| `mcp-server-langgraph` (Service) | stg | `stg-mcp-server-langgraph` |
| `mcp-server-langgraph-config` (ConfigMap) | stg | `stg-mcp-server-langgraph-config` |
| `mcp-server-langgraph-secrets` (Secret) | stg | `stg-mcp-server-langgraph-secrets` |
...
Preview:      us-central1-docker.pkg.dev/vishnu-sandbox-20250310/mcp-stg/mcp-server-langgraph
...
STG:          2.8.0-stg, stg-latest, stg-{sha}
...
| `GKE_PREVIEW_CLUSTER` | `stg-mcp-server-langgraph-gke` | STG GKE cluster |
...
| Variable | Default | Description |
|----------|---------|-------------|
| `PREVIEW_NAMESPACE` | `stg-mcp-server-langgraph` | Preview namespace |
...
|----------|-------|-------------|
| `DEPLOYMENT_NAME (stg) | `stg-mcp-server-langgraph` | STG deployment |
...
NAMESPACE="stg-mcp-server-langgraph"
SERVICE_NAME="stg-mcp-server-langgraph"
...
|-------------|-------------|--------|
| `mcp-stg-cluster` | `stg-mcp-server-langgraph-gke` | ❌ Deprecated |
| `mcp-prod-gke` | `prod-mcp-server-langgraph-gke` | ❌ Deprecated |
| `mcp-stg` | `stg-mcp-server-langgraph` | ❌ Deprecated |
| `mcp-prod` | `prod-mcp-server-langgraph` | ❌ Deprecated |
| `mcp-server-langgraph-stg` | `stg-mcp-server-langgraph` | ❌ Deprecated |
| `mcp-dev-cluster` | `dev-mcp-server-langgraph-gke` | ❌ Deprecated |
| `stg-*` | `preview-*` | ❌ Deprecated (use preview-) |

### Migration Status

| Component | Migration Status | Updated Date |
|-----------|-----------------|--------------|
| GCP Terraform (stg, prod) | ✅ Complete | 2025-11-04 |
| GCP Terraform (dev) | ✅ Complete | 2025-11-04 |
| AWS Terraform (dev, stg) | ✅ Complete | 2025-11-04 |
...
1. **Environment-First**: Always start with the environment name (`dev`, `stg`, `production`)
...
- name: Validate Naming Conventions
  run: |
    ...
    if grep -r "mcp-stg-cluster\|mcp-prod-gke" terraform/ deployments/ scripts/; then
