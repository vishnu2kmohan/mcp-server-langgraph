...
description: Required secrets and variables for GitHub Actions workflows
icon: 'diagram-project'
contentType: explanation
seoTitle: GitHub Workflows Secrets - MCP Server LangGraph
seoDescription: Required secrets and variables for GitHub Actions workflows Learn
...
#### `GCP_STG_SA_EMAIL`
- **Purpose**: Service account email for stg deployments
- **Format**: `stg-deploy@PROJECT_ID.iam.gserviceaccount.com`
- **Used By**: `deploy-gke-stg.yaml`
...
- **Setup**:
  ...
  gcloud iam service-accounts create stg-deploy \
    --display-name="STG Deployment Service Account"

  # Grant permissions
  gcloud projects add-iam-policy-binding YOUR_PROJECT_ID \
    --member="serviceAccount:stg-deploy@YOUR_PROJECT_ID.iam.gserviceaccount.com" \
    --role="roles/container.developer"

  # Bind to Workload Identity
  gcloud iam service-accounts add-iam-policy-binding \
    stg-deploy@YOUR_PROJECT_ID.iam.gserviceaccount.com \
...
- **Permissions Required**: Same as stg SA
- **Setup**: Similar to stg SA
...
### `ENABLE_STG_AUTODEPLOY`
- **Purpose**: Enable/disable automatic stg deployments on main push
...
- [ ] **GCP Authentication**
  ...
  - [ ] Create stg and production service accounts
  - [ ] Bind service accounts to Workload Identity
  - [ ] Add `GCP_WIF_PROVIDER`, `GCP_STG_SA_EMAIL`, `GCP_PROD_SA_EMAIL` secrets
...
- [ ] **Deployment Automation** (optional)
  - [ ] Set `ENABLE_STG_AUTODEPLOY` variable to `true`
...
```bash
# Example: STG SA only needs GKE and Artifact Registry access
gcloud projects add-iam-policy-binding YOUR_PROJECT_ID \
  --member="serviceAccount:stg-deploy@YOUR_PROJECT_ID.iam.gserviceaccount.com" \
  --role="roles/container.developer"
...
gcloud logging read "protoPayload.authenticationInfo.principalEmail:stg-deploy@" \
  --limit=50 \
  --format=json
```
