...
<AccordionGroup>
  ...
  <Accordion title="Rule 3: Stdlib Backports" icon="3">
    Python 3.11+ includes `tomllib` in stdlib. For older versions, tests may import `tomli`:
    ...
    The regression test allows `tomli` imports if `tomllib` is in stdlib (Python 3.12+).
  </Accordion>
