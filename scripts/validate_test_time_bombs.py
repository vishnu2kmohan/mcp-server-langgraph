...
# Allowed exceptions (constants that are clearly test/fake values)
ALLOWED_PATTERNS = [
    r"TEST_.*MODEL",
    r"FAKE_.*MODEL",
    r"NONEXISTENT",
...
def check_file(filepath: Path) -> List[Dict[str, Any]]:
    ...
    max_safe_year = current_year + 2  # Allow up to 2 years in the future
                    ...
                    issues.append(
                        {
                            ...
                            "suggestion": "Use clearly fake test constants (e.g., TEST_NONEXISTENT_MODEL = 'gpt-999-test-nonexistent')",
                        }
                    )
