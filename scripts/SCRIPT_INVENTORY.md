...
|--------|----------|----------|
| `stg-smoke-tests.sh` | shellcheck | deploy-stg-gke |
...
| Script | Workflows |
|--------|----------|
| `setup-stg-infrastructure.sh` | deploy-stg-gke, validate-deployments |
...
| `check-links.py` | docs-validation, link-checker |
| `setup-stg-infrastructure.sh` | deploy-stg-gke, validate-deployments |
| `health-check.sh` | ci, deploy-stg-gke |
...
**gcp/**: 4 scripts
  - `setup-vertex-ai-stg.sh`
  - `setup-workload-identity.sh`
  - `teardown-stg-infrastructure.sh`
  - `validate-stg-deployment.sh`
