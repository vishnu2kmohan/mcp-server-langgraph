...
def validate_pre_commit_config(repo_root: Path) -> tuple[bool, list[str]]:
    ...
    if not pre_push_hooks:
        errors.append(
            "❌ No hooks configured with 'stages: [pre-push]' in .pre-commit-config.yaml\n"
    ...
    found_optional = {category: False for category in optional_patterns}
    warnings = []
...
def check_pre_push_hook() -> tuple[bool, list[str]]:
    ...
    if is_pre_commit_wrapper(content):
        ...
        print("ℹ️  Detected pre-commit framework wrapper")
    ...
    print("ℹ️  Detected custom pre-push script")
