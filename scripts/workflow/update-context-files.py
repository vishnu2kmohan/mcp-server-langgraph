...
# Update recent-work.md in background (non-blocking for fast commits)
# Uses project-managed Python runtime for consistency
uv run python scripts/workflow/update-context-files.py --recent-work --quiet &
"""

    if hook_path.exists():
        print(f"⚠️  Git hook already exists: {hook_path}")
        print("   To append auto-update, add this line:")
        print("   uv run python scripts/workflow/update-context-files.py --recent-work --quiet &")
    else:
