...
This script scans test files to ensure they follow memory safety patterns that
prevent memory explosion when running tests with pytest-xdist parallel execution.
            ...
            if not has_xdist_skipif:
                self.violations.append(
                    Violation(
                        ...
                        fix_suggestion=f'Add @pytest.mark.skipif(os.getenv("PYTEST_XDIST_WORKER") is not None, reason="...") above test method',
                    )
                )
...
def main() -> int:
    ...
    if len(sys.argv) > 1:
        ...
        files_to_check = find_test_files(str(repo_root / "tests"))
