...
###############################################################################
# GKE STG Environment - Teardown Script
#
# Single-command teardown of the complete GKE stg environment:
...
#
# Usage:
#   ./scripts/gcp/gke-stg-down.sh [OPTIONS]
#
# Options:
...
#   ./scripts/gcp/gke-stg-down.sh --project my-project-id
#
#   # Non-interactive (for CI/CD)
#   ./scripts/gcp/gke-stg-down.sh --auto-approve
#
...
Completely teardown the GKE stg environment with a single command.
...
list_resources() {
    ...
    if gcloud artifacts repositories describe "mcp-stg" \
        --location="$REGION" --project="$PROJECT_ID" &> /dev/null; then
        found_resources+=("Artifact Registry: mcp-stg")
    fi

    # Check DNS Zone
    if gcloud dns managed-zones describe "stg-internal" \
        --project="$PROJECT_ID" &> /dev/null; then
        found_resources+=("DNS Zone: stg-internal")
    fi
    ...
    if [ ${#found_resources[@]} -eq 0 ]; then
        log_warn "No stg resources found in project $PROJECT_ID"
        return 1
    fi
...
confirm_teardown() {
    ...
    echo "║                     ⚠️  WARNING ⚠️                              ║"
    echo "║                                                                ║"
    echo "║  This will PERMANENTLY DELETE all stg environment         ║"
...
terraform_destroy() {
    ...
    if ! check_terraform_state; then
        ...
        if [ -x "$SCRIPT_DIR/teardown-stg-infrastructure.sh" ]; then
            if [ "$AUTO_APPROVE" = true ]; then
                "$SCRIPT_DIR/teardown-stg-infrastructure.sh" --auto-approve
            else
                "$SCRIPT_DIR/teardown-stg-infrastructure.sh"
            fi
        else
...
verify_cleanup() {
    ...
    if gcloud artifacts repositories describe "mcp-stg" \
        --location="$REGION" --project="$PROJECT_ID" &> /dev/null; then
        remaining_resources+=("Artifact Registry: mcp-stg")
    fi

    # Check DNS Zone
    if gcloud dns managed-zones describe "stg-internal" \
        --project="$PROJECT_ID" &> /dev/null; then
        remaining_resources+=("DNS Zone: stg-internal")
    fi
...
show_completion_message() {
    echo ""
    show_success_banner "GKE STG Environment Teardown Complete!"

    echo ""
    echo "All stg environment resources have been deleted."
    echo ""
    echo "To recreate the environment:"
    echo "   ./scripts/gcp/gke-stg-up.sh"
    echo ""
    echo "Or using make:"
    echo "   make gke-stg-up"
    echo ""
}
...
main() {
    show_header "GKE STG Environment Teardown" "Complete cleanup of stg infrastructure"
