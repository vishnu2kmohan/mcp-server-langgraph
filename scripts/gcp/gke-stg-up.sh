...
###############################################################################
# GKE STG Environment - Setup Script
#
# Single-command deployment of the complete GKE stg environment:
...
#
# Usage:
#   ./scripts/gcp/gke-stg-up.sh [OPTIONS]
#
# Options:
...
#   ./scripts/gcp/gke-stg-up.sh --project my-project-id
#
#   # Or using environment variable
#   GCP_PROJECT_ID=my-project-id ./scripts/gcp/gke-stg-up.sh
#
...
SHORT_PREFIX="stg-mcp-slg"  # Used in derived names below
...
Deploy the complete GKE stg environment with a single command.
...
For more details, see: docs/deployment/kubernetes/gke-stg.mdx
...
deploy_kubernetes() {
    ...
    kubectl apply -k "$PROJECT_ROOT/deployments/overlays/stg-gke"
...
run_validation() {
    ...
    # Run smoke tests
    if [ -x "$SCRIPT_DIR/stg-smoke-tests.sh" ]; then
        log_substep "Running smoke tests..."
        "$SCRIPT_DIR/stg-smoke-tests.sh" --skip-integration-tests || {
            log_warn "Some smoke tests failed"
        }
    fi

    # Run validation script
    if [ -x "$SCRIPT_DIR/validate-stg-deployment.sh" ]; then
        log_substep "Running deployment validation..."
        "$SCRIPT_DIR/validate-stg-deployment.sh" --skip-cost-check || {
...
show_access_instructions() {
    echo ""
    show_success_banner "GKE STG Environment Deployed Successfully!"
    ...
    echo ""
    echo "3. View logs:"
    echo "   kubectl logs -f deployment/stg-mcp-server-langgraph -n $NAMESPACE"
    echo ""
    echo "4. Port-forward to services:"
    echo "   kubectl port-forward svc/stg-mcp-server-langgraph 8000:8000 -n $NAMESPACE"
    echo "   kubectl port-forward svc/stg-keycloak 8082:8082 -n $NAMESPACE"
    echo "   kubectl port-forward svc/stg-openfga 8080:8080 -n $NAMESPACE"
    echo ""
    ...
    echo "   ./scripts/gcp/gke-stg-down.sh"
    echo ""
    echo "Or using make:"
    echo "   make gke-stg-down"
    echo ""
}
...
main() {
    show_header "GKE STG Environment Setup" "Deploy complete stg infrastructure"
    ...
    log_success "GKE stg environment is ready!"
}
