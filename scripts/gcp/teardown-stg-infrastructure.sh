...
###############################################################################
# stg GKE Infrastructure Teardown Script
#
# This script performs a complete teardown of the stg GKE environment,
...
#
# Usage:
#   ./scripts/gcp/teardown-stg-infrastructure.sh [OPTIONS]
#
# Options:
...
readonly CLUSTER_NAME="stg-mcp-server-langgraph-gke"
readonly NAMESPACE="stg-mcp-server-langgraph"
# VPC and resource names match Terraform naming convention (short_prefix = "stg-mcp-slg")
...
readonly TERRAFORM_DIR="terraform/environments/gcp-stg"
readonly ARTIFACT_REGISTRY_REPO="mcp-stg"
...
readonly SECRETS=(
    ...
    "stg-keycloak-db-password"
    "stg-openfga-db-password"
    "stg-gdpr-db-password"
    "stg-postgres-username"
    "stg-gdpr-postgres-url"
    # Redis configuration
    "stg-redis-password"
    "stg-redis-host"
    # API keys
    "stg-anthropic-api-key"
    "stg-google-api-key"
    "stg-jwt-secret"
    "stg-qdrant-api-key"
    # Keycloak configuration
    "stg-keycloak-admin-password"
    "stg-keycloak-admin-username"
    "stg-keycloak-client-id"
    "stg-keycloak-client-secret"
    # OpenFGA configuration
    "stg-openfga-model-id"
    "stg-openfga-store-id"
    # Infisical (secrets management)
    "stg-infisical-client-id"
    "stg-infisical-client-secret"
    "stg-infisical-project-id"
)
...
confirm_teardown() {
    ...
    log_warn "============================================"
    log_warn "WARNING: This will DELETE all stg resources"
    log_warn "============================================"
...
cleanup_monitoring_alerts() {
    ...
    # Find all alert policies containing "stg" in the display name
    log_info "Searching for stg monitoring alert policies..."
    local alert_policies
    alert_policies=$(gcloud alpha monitoring policies list \
        --project="$PROJECT_ID" \
        --filter="displayName~stg" \
        --format="value(name)" 2>/dev/null || echo "")

    if [ -z "$alert_policies" ]; then
        log_warn "No stg monitoring alert policies found"
        return 0
    fi
...
delete_vpc_network() {
    ...
    # The address name is: ${name_prefix}-private-services (e.g., stg-mcp-slg-private-services)
...
verify_cleanup() {
    ...
    # Check DNS zone
    if gcloud dns managed-zones describe "stg-internal" \
        --project="$PROJECT_ID" &> /dev/null; then
        failed_checks+=("DNS zone stg-internal still exists")
    fi
    ...
    if [ "$remaining_policies" -gt 0 ]; then
        failed_checks+=("$remaining_policies stg monitoring alert policies still exist")
    fi
...
main() {
    log_info "Starting stg GKE infrastructure teardown..."
    ...
    log_success "=========================================="
    log_success "stg infrastructure teardown complete!"
    log_success "=========================================="
}
