#!/bin/bash
#
# Vertex AI Workload Identity Setup for stg
#
# This script configures Workload Identity Federation for Vertex AI access
# in the stg GKE environment. It creates a dedicated service account
# and binds it to the Kubernetes service account for keyless authentication.
#
# Prerequisites:
# - GKE cluster with Workload Identity enabled
# - gcloud CLI installed and authenticated
# - stg infrastructure already created (run setup-stg-infrastructure.sh first)
#
# Usage:
...
REGION="us-central1"
CLUSTER_NAME="mcp-stg-cluster"
K8S_NAMESPACE="mcp-stg"
K8S_SERVICE_ACCOUNT="mcp-server-langgraph"
GCP_SERVICE_ACCOUNT="vertex-ai-stg"
...
check_prerequisites() {
    ...
    if ! gcloud container clusters describe "$CLUSTER_NAME" --region="$REGION" &> /dev/null; then
        log_error "GKE cluster '$CLUSTER_NAME' not found. Run setup-stg-infrastructure.sh first."
        exit 1
    fi
    ...
    if ! kubectl get namespace "$K8S_NAMESPACE" &> /dev/null; then
        log_error "Kubernetes namespace '$K8S_NAMESPACE' not found. Deploy stg manifests first."
        exit 1
    fi
}
...
create_service_account() {
    ...
    gcloud iam service-accounts create "$GCP_SERVICE_ACCOUNT" \
        ...
        --description="Service account for Vertex AI access in stg environment" \
        --project="$PROJECT_ID"
...
annotate_kubernetes_service_account() {
    ...
    if ! kubectl get serviceaccount "$K8S_SERVICE_ACCOUNT" -n "$K8S_NAMESPACE" &> /dev/null; then
        ...
        log_error "Deploy the stg manifests first: kubectl apply -k deployments/overlays/stg-gke"
        exit 1
    fi
...
print_summary() {
    ...
    echo "     kubectl apply -k deployments/overlays/stg-gke"
    echo ""
