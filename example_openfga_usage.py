...
async def demo_authorization():
    ...
    result = await auth.authorize(
        user_id="user:alice",
        relation="owner",
        resource="conversation:thread_1"
    )
    ...
    tools = await auth.list_accessible_resources(
        user_id="user:alice",
        relation="executor",
        resource_type="tool"
    )
    ...
    conversations = await auth.list_accessible_resources(
        user_id="user:bob",
        relation="viewer",
    ...
    result = await auth.authorize(
        user_id="user:charlie",
        relation="viewer",
        resource="conversation:thread_2"
    )
    ...
    expansion = await client.expand_relation(
        relation="executor",
        object="tool:chat"
    )
