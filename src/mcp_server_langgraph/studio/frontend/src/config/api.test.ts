...
describe("API Configuration", () => {
  ...
  describe("buildWebSocketUrl", () => {
    it("should append endpoint path to WebSocket base URL", async () => {
      const { buildWebSocketUrl, API_ENDPOINTS } = await import("./api");
      ...
      const url = buildWebSocketUrl(API_ENDPOINTS.WS_NOTIFICATIONS, mockWindow);

      expect(url).toBe("wss://app.example.com/api/v1/ws/notifications");
    });

    it("should append token query parameter when provided", async () => {
      const { buildWebSocketUrl, API_ENDPOINTS } = await import("./api");
      ...
      const url = buildWebSocketUrl(
        API_ENDPOINTS.WS_ALERTS,
        mockWindow,
        "my-token-123",
      );

      expect(url).toBe(
        "wss://app.example.com/api/v1/ws/alerts?token=my-token-123",
      );
    });
