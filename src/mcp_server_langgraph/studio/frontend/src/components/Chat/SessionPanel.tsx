...
export function SessionPanel({
  ...
  const handleRenameSubmit = async (sessionId: string) => {
    if (editingName.trim() && onRename) {
      await onRename(sessionId, editingName.trim());
    }
                  ...
                  {editingSessionId === session.id ? (
                    <div className="flex-1 flex items-center gap-1 min-w-0">
                      <input
                        ...
                        autoFocus
                        className="flex-1 px-2 py-1 text-sm border border-blue-500 rounded bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-1 focus:ring-blue-500"
                        ...
                        {openMenuId === session.id && (
                          <div className="absolute right-0 mt-1 w-32 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded shadow-lg z-50">
                            {onRename && (
                              <button
                                onClick={(e) =>
                                  handleRenameClick(session.id, session.name, e)
                                }
