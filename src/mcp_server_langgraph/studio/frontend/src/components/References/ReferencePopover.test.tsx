...
// Test fixtures
const createToolReference = (
  overrides: Partial<ResolvedReference> = {},
): ResolvedReference => ({
  type: "tool",
...
describe("ReferencePopover", () => {
  ...
  describe("rendering", () => {
    ...
    it("should display the description when available", () => {
      const ref = createToolReference({
        description: "A powerful file reading tool",
      });
      renderWithRouter(
        <ReferencePopover reference={ref} isOpen={true} onClose={() => {}} />,
      );
      ...
      expect(
        screen.getByText("A powerful file reading tool"),
      ).toBeInTheDocument();
    });
  ...
  describe("deep linking", () => {
    it("should show deep link for valid tool references", () => {
      ...
      expect(link).toHaveAttribute(
        "href",
        "/connections?selected=conn-abc&tab=capabilities",
      );
    });
