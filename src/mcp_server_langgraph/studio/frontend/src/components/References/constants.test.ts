...
describe("toMarkdownReference", () => {
  ...
  it("should include label when provided", () => {
    expect(
      toMarkdownReference("tool", "filesystem", "read_file", "Read File"),
    ).toBe("[[tool:filesystem:read_file|Read File]]");
  });

  it("should include label for non-tool types", () => {
    expect(
      toMarkdownReference("skill", "code-review", undefined, "Code Review"),
    ).toBe("[[skill:code-review|Code Review]]");
  });
});
...
describe("parseMarkdownReference", () => {
  ...
  it("should parse reference with label", () => {
    const result = parseMarkdownReference(
