...
export function InstallPrompt({
  ...
  return (
    <div
      role="alert"
      aria-live="polite"
      className="fixed bottom-4 left-1/2 -translate-x-1/2 z-70 bg-success-10 text-neutral-12 px-4 py-3 rounded-lg shadow-lg flex items-center gap-3 max-w-md"
    >
      <Download
        className={`h-5 w-5 flex-shrink-0 ${isInstalling ? "animate-bounce motion-reduce:animate-none" : ""}`}
        aria-hidden="true"
      />
      <div className="flex-1 min-w-0">
        ...
        {!isInstalling && (
          <p className="text-xs text-success-3 mt-0.5">
      ...
      <div className="flex items-center gap-2 flex-shrink-0">
        <Button
          variant="secondary"
          className="px-3 py-1.5 text-sm bg-neutral-1 text-success-10 rounded hover:bg-success-2"
          type="button"
          onClick={onInstall}
        ...
        <Button
          variant="success"
          className="p-1.5 text-success-3 hover:text-neutral-12 rounded"
          type="button"
          onClick={onDismiss}
          disabled={isInstalling}
