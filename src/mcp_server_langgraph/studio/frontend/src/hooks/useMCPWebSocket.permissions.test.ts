/**
 * Permission-based Connection Control Tests for useMCPWebSocket
 *
 * Security requirement: WebSocket connections must only be established when:
 * 1. User is authenticated (isAuthenticated=true)
 * 2. User has mcp_aggregated permission (wsPermissions.mcp_aggregated=true)
 *
...
let mockWsPermissions: { mcp_aggregated: boolean } | null = {
  mcp_aggregated: true,
};
...
describe("useMCPWebSocket permission-based connection control", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    capturedUrl = "";
    // Reset to default authenticated state with permission
    mockIsAuthenticated = true;
    mockWsPermissions = { mcp_aggregated: true };
  });
  ...
  it("should NOT create WebSocket URL when mcp_aggregated permission is denied", () => {
    // Mock: authenticated=true but permission=false
    mockIsAuthenticated = true;
    mockWsPermissions = { mcp_aggregated: false };
  ...
  it("should create WebSocket URL when authenticated AND has permission", () => {
    ...
    expect(capturedUrl).toContain("/api/v1/ws/mcp");
  });
