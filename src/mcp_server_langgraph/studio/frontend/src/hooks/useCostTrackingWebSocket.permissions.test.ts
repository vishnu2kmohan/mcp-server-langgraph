/**
 * Permission-based Connection Control Tests for useCostTrackingWebSocket
 *
 * Security requirement: WebSocket connections must only be established when:
 * 1. User is authenticated (isAuthenticated=true)
 * 2. User has cost_tracking permission (wsPermissions.cost_tracking=true)
 *
...
let mockWsPermissions: { cost_tracking: boolean } | null = {
  cost_tracking: true,
};
...
describe("useCostTrackingWebSocket permission-based connection control", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    capturedUrl = "";
    // Reset to default authenticated state with permission
    mockIsAuthenticated = true;
    mockWsPermissions = { cost_tracking: true };
  });
  ...
  it("should NOT create WebSocket URL when cost_tracking permission is denied", () => {
    // Mock: authenticated=true but permission=false
    mockIsAuthenticated = true;
    mockWsPermissions = { cost_tracking: false };
  ...
  it("should create WebSocket URL when authenticated AND has permission", () => {
    ...
    expect(capturedUrl).toContain("/api/v1/ws/usage/cost");
  });
