/**
 * Permission-based Connection Control Tests for useAgentRequestWebSocket
 *
 * Security requirement: WebSocket connections must only be established when:
 * 1. User is authenticated (isAuthenticated=true)
 * 2. User has agent_requests permission (wsPermissions.agent_requests=true)
 *
...
let mockWsPermissions: { agent_requests: boolean } | null = {
  agent_requests: true,
};
...
describe("useAgentRequestWebSocket permission-based connection control", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    capturedUrl = "";
    // Reset to default authenticated state with permission
    mockIsAuthenticated = true;
    mockWsPermissions = { agent_requests: true };
  });
  ...
  it("should NOT create WebSocket URL when agent_requests permission is denied", () => {
    // Mock: authenticated=true but permission=false
    mockIsAuthenticated = true;
    mockWsPermissions = { agent_requests: false };
  ...
  it("should create WebSocket URL when authenticated AND has permission", () => {
    ...
    expect(capturedUrl).toContain("/api/v1/ws/agents/requests");
  });
