/**
 * Permission-based Connection Control Tests for useHeartMetricsWebSocket
 *
 * Security requirement: WebSocket connections must only be established when:
 * 1. User is authenticated (isAuthenticated=true)
 * 2. User has heart_metrics permission (wsPermissions.heart_metrics=true)
 *
...
let mockWsPermissions: { heart_metrics: boolean } | null = {
  heart_metrics: true,
};
...
describe("useHeartMetricsWebSocket permission-based connection control", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    capturedUrl = "";
    // Reset to default authenticated state with permission
    mockIsAuthenticated = true;
    mockWsPermissions = { heart_metrics: true };
  });
  ...
  it("should NOT create WebSocket URL when heart_metrics permission is denied", () => {
    // Mock: authenticated=true but permission=false
    mockIsAuthenticated = true;
    mockWsPermissions = { heart_metrics: false };

    renderHook(() => useHeartMetricsWebSocket());
  ...
  it("should create WebSocket URL when authenticated AND has permission", () => {
    ...
    expect(capturedUrl).toContain("/api/v1/ws/metrics/heart");
  });
