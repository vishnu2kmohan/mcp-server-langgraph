/**
 * Permission-based Connection Control Tests for useLLMStreamingWebSocket
 *
 * Security requirement: WebSocket connections must only be established when:
 * 1. User is authenticated (isAuthenticated=true)
 * 2. User has llm_streaming permission (wsPermissions.llm_streaming=true)
 *
...
import { useLLMStreamingWebSocket } from "./useLLMStreamingWebSocket";
...
let mockWsPermissions: { llm_streaming: boolean } | null = {
  llm_streaming: true,
};
...
describe("useLLMStreamingWebSocket permission-based connection control", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    capturedUrl = "";
    // Reset to default authenticated state with permission
    mockIsAuthenticated = true;
    mockWsPermissions = { llm_streaming: true };
  });
  ...
  it("should NOT create WebSocket URL when llm_streaming permission is denied", () => {
    // Mock: authenticated=true but permission=false
    mockIsAuthenticated = true;
    mockWsPermissions = { llm_streaming: false };

    renderHook(() => useLLMStreamingWebSocket());
  ...
  it("should create WebSocket URL when authenticated AND has permission", () => {
    ...
    expect(capturedUrl).toContain("/api/v1/ws/llm/streaming");
  });
