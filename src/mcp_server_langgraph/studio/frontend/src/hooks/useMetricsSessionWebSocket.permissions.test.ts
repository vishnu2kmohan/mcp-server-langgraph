/**
 * Permission-based Connection Control Tests for useMetricsSessionWebSocket
 *
 * Security requirement: WebSocket connections must only be established when:
 * 1. User is authenticated (isAuthenticated=true)
 * 2. User has session_metrics permission (wsPermissions.session_metrics=true)
 *
...
import { useMetricsSessionWebSocket } from "./useMetricsSessionWebSocket";
...
let mockWsPermissions: { session_metrics: boolean } | null = {
...
describe("useMetricsSessionWebSocket permission-based connection control", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    capturedUrl = "";
    // Reset to default authenticated state with permission
    mockIsAuthenticated = true;
    mockWsPermissions = { session_metrics: true };
  });
  ...
  it("should NOT create WebSocket URL when session_metrics permission is denied", () => {
    // Mock: authenticated=true but permission=false
    mockIsAuthenticated = true;
    mockWsPermissions = { session_metrics: false };

    renderHook(() => useMetricsSessionWebSocket());
  ...
  it("should create WebSocket URL when authenticated AND has permission", () => {
    ...
    expect(capturedUrl).toContain("/api/v1/ws/metrics/session");
  });
