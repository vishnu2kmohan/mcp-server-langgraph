/**
 * Permission-based Connection Control Tests for useAlertWebSocket
 *
 * Security requirement: WebSocket connections must only be established when:
 * 1. User is authenticated (isAuthenticated=true)
 * 2. User has alerts permission (wsPermissions.alerts=true)
 *
...
let mockIsInitializing = false; // useAlertWebSocket checks isAuthReady = isAuthenticated && !isInitializing
let mockWsPermissions: { alerts: boolean } | null = {
  alerts: true,
};
...
describe("useAlertWebSocket permission-based connection control", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    capturedUrl = "";
  ...
  it("should NOT create WebSocket URL when alerts permission is denied", () => {
    // Mock: authenticated=true but permission=false
    mockIsAuthenticated = true;
    mockWsPermissions = { alerts: false };
  ...
  it("should create WebSocket URL when authenticated AND has permission", () => {
    ...
    expect(capturedUrl).toContain("/api/v1/ws/alerts");
  });
