/**
 * Permission-based Connection Control Tests for useMCPTaskWebSocket
 *
 * Security requirement: WebSocket connections must only be established when:
 * 1. User is authenticated (isAuthenticated=true)
 * 2. User has mcp_tasks permission (wsPermissions.mcp_tasks=true)
 *
...
import { useMCPTaskWebSocket } from "./useMCPTaskWebSocket";
...
let mockWsPermissions: { mcp_tasks: boolean } | null = {
  mcp_tasks: true,
};
...
describe("useMCPTaskWebSocket permission-based connection control", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    capturedUrl = "";
    // Reset to default authenticated state with permission
    mockIsAuthenticated = true;
    mockWsPermissions = { mcp_tasks: true };
  });
  ...
  it("should NOT create WebSocket URL when mcp_tasks permission is denied", () => {
    // Mock: authenticated=true but permission=false
    mockIsAuthenticated = true;
    mockWsPermissions = { mcp_tasks: false };
  ...
  it("should create WebSocket URL when authenticated AND has permission", () => {
    ...
    expect(capturedUrl).toContain("/api/v1/ws/mcp/tasks");
  });
