/**
 * Permission-based Connection Control Tests for useNotificationWebSocket
 *
 * Security requirement: WebSocket connections must only be established when:
 * 1. User is authenticated (isAuthenticated=true)
 * 2. User has notifications permission (wsPermissions.notifications=true)
 *
...
let mockIsInitializing = false; // useNotificationWebSocket checks isAuthReady = isAuthenticated && !isInitializing
let mockWsPermissions: { notifications: boolean } | null = {
  notifications: true,
};
...
describe("useNotificationWebSocket permission-based connection control", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    capturedUrl = "";
    // Reset to default authenticated state with permission
    mockIsAuthenticated = true;
    mockIsInitializing = false; // Auth initialization complete
    mockWsPermissions = { notifications: true };
  });
  ...
  it("should NOT create WebSocket URL when notifications permission is denied", () => {
    // Mock: authenticated=true but permission=false
    mockIsAuthenticated = true;
    mockWsPermissions = { notifications: false };
  ...
  it("should create WebSocket URL when authenticated AND has permission", () => {
    ...
    expect(capturedUrl).toContain("/api/v1/ws/notifications");
  });
