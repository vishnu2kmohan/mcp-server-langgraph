/**
 * Permission-based Connection Control Tests for useAISuggestionsWebSocket
 *
 * Security requirement: WebSocket connections must only be established when:
 * 1. User is authenticated (isAuthenticated=true)
 * 2. User has ai_suggestions permission (wsPermissions.ai_suggestions=true)
 *
...
let mockWsPermissions: { ai_suggestions: boolean } | null = {
  ai_suggestions: true,
};
...
describe("useAISuggestionsWebSocket permission-based connection control", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    capturedUrl = "";
    // Reset to default authenticated state with permission
    mockIsAuthenticated = true;
    mockWsPermissions = { ai_suggestions: true };
  });
  ...
  it("should NOT create WebSocket URL when ai_suggestions permission is denied", () => {
    // Mock: authenticated=true but permission=false
    mockIsAuthenticated = true;
    mockWsPermissions = { ai_suggestions: false };

    renderHook(() => useAISuggestionsWebSocket());
