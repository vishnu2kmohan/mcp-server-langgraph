/**
 * Permission-based Connection Control Tests for useAIOrchestratorStatus
 *
 * Security requirement: WebSocket connections must only be established when:
 * 1. User is authenticated (isAuthenticated=true)
 * 2. User has orchestrator_status permission (wsPermissions.orchestrator_status=true)
 *
...
import { useAIOrchestratorStatus } from "./useAIOrchestratorStatus";
...
let mockWsPermissions: { orchestrator_status: boolean } | null = {
...
describe("useAIOrchestratorStatus permission-based connection control", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    capturedUrl = "";
    // Reset to default authenticated state with permission
    mockIsAuthenticated = true;
    mockWsPermissions = { orchestrator_status: true };
  });
  ...
  it("should NOT create WebSocket URL when orchestrator_status permission is denied", () => {
    // Mock: authenticated=true but permission=false
    mockIsAuthenticated = true;
    mockWsPermissions = { orchestrator_status: false };

    renderHook(() => useAIOrchestratorStatus());
  ...
  it("should create WebSocket URL when authenticated AND has permission", () => {
    ...
    expect(capturedUrl).toContain("/api/v1/ws/orchestrator/status");
  });
