/**
 * Permission-based Connection Control Tests for useConnectionsRealtimeWebSocket
 *
 * Security requirement: WebSocket connections must only be established when:
 * 1. User is authenticated (isAuthenticated=true)
 * 2. User has connections_realtime permission (wsPermissions.connections_realtime=true)
 *
...
let mockWsPermissions: { connections_realtime: boolean } | null = {
...
describe("useConnectionsRealtimeWebSocket permission-based connection control", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    capturedUrl = "";
    // Reset to default authenticated state with permission
    mockIsAuthenticated = true;
    mockWsPermissions = { connections_realtime: true };
  });
  ...
  it("should NOT create WebSocket URL when connections_realtime permission is denied", () => {
    // Mock: authenticated=true but permission=false
    mockIsAuthenticated = true;
    mockWsPermissions = { connections_realtime: false };
  ...
  it("should create WebSocket URL when authenticated AND has permission", () => {
    ...
    expect(capturedUrl).toContain("/api/v1/ws/connections/realtime");
  });
