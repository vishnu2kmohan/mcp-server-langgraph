...
export function useMCPTaskWebSocket(
  ...
  // Check if user has permission for MCP tasks WebSocket
  const hasMcpTasksPermission = wsPermissions?.mcp_tasks ?? false;
  ...
  const url = useMemo(
    () =>
      isAuthenticated && hasMcpTasksPermission
        ? (customUrl ?? buildWebSocketUrl(WS_ENDPOINTS.MCP_TASKS, {}, true))
        : "",
    // authToken dependency ensures URL regenerates when token is refreshed
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [customUrl, authToken, isAuthenticated, hasMcpTasksPermission],
  );
