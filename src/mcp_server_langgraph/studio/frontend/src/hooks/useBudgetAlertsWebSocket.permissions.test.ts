/**
 * Permission-based Connection Control Tests for useBudgetAlertsWebSocket
 *
 * Security requirement: WebSocket connections must only be established when:
 * 1. User is authenticated (isAuthenticated=true)
 * 2. User has budget_alerts permission (wsPermissions.budget_alerts=true)
 *
...
let mockWsPermissions: { budget_alerts: boolean } | null = {
  budget_alerts: true,
};
...
describe("useBudgetAlertsWebSocket permission-based connection control", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    capturedUrl = "";
    // Reset to default authenticated state with permission
    mockIsAuthenticated = true;
    mockWsPermissions = { budget_alerts: true };
  });
  ...
  it("should NOT create WebSocket URL when budget_alerts permission is denied", () => {
    // Mock: authenticated=true but permission=false
    mockIsAuthenticated = true;
    mockWsPermissions = { budget_alerts: false };

    renderHook(() => useBudgetAlertsWebSocket());
  ...
  it("should create WebSocket URL when authenticated AND has permission", () => {
    ...
    expect(capturedUrl).toContain("/api/v1/ws/budget/alerts");
  });
