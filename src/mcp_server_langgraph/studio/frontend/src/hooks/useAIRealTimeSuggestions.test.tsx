...
describe('useAIRealTimeSuggestions', () => {
  ...
  describe('Suggestion Handling', () => {
    ...
    it('should handle multiple suggestions', async () => {
      ...
      act(() => {
        mockOnMessage?.({
          type: 'suggestions',
          data: [
            { id: 'nudge-1', type: 'tooltip', message: 'Tip 1', priority: 'low' },
            { id: 'nudge-2', type: 'spotlight', message: 'Tip 2', priority: 'high' },
          ],
        });
      });
    ...
    it('should clear suggestions when requested', async () => {
      ...
      act(() => {
        mockOnMessage?.({
          type: 'suggestions',
          data: [{ id: 'nudge-1', type: 'tooltip', message: 'Tip', priority: 'low' }],
        });
      });
  ...
  describe('Request Suggestions', () => {
    ...
    it('should include context in request', () => {
      ...
      act(() => {
        result.current.requestSuggestions({ page: 'workflows', action: 'create' });
      });
  ...
  describe('Suggestion Dismissal', () => {
    it('should dismiss a specific suggestion', async () => {
      ...
      act(() => {
        mockOnMessage?.({
          ...
          data: [
            { id: 'nudge-1', type: 'tooltip', message: 'Tip 1', priority: 'low' },
            { id: 'nudge-2', type: 'tooltip', message: 'Tip 2', priority: 'high' },
          ],
        });
      });
    ...
    it('should not show previously dismissed suggestions', async () => {
      ...
      act(() => {
        mockOnMessage?.({
          type: 'suggestions',
          data: [{ id: 'nudge-1', type: 'tooltip', message: 'Tip 1', priority: 'low' }],
        });
      });
