/**
 * Permission-based Connection Control Tests for useConnectionHealthWebSocket
 *
 * Separate test file to allow different mock configurations for testing
 * permission-based WebSocket URL generation.
 *
 * Security requirement: WebSocket connections must only be established when:
 * 1. User is authenticated (isAuthenticated=true)
 * 2. User has connections_health permission (wsPermissions.connections_health=true)
 *
...
import { useConnectionHealthWebSocket } from "./useConnectionHealthWebSocket";
...
let mockWsPermissions: { connections_health: boolean } | null = {
...
vi.mock("../store/hooks", () => ({
  ...
  useAppSelector: vi.fn((selector) => {
    // Check selector function name to determine which value to return
...
describe("useConnectionHealthWebSocket permission-based connection control", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    capturedUrl = "";
    // Reset to default authenticated state with permission
    mockIsAuthenticated = true;
    mockWsPermissions = { connections_health: true };
  });
  ...
  it("should NOT create WebSocket URL when connections_health permission is denied", () => {
    // Mock: authenticated=true but permission=false
    mockIsAuthenticated = true;
    mockWsPermissions = { connections_health: false };
  ...
  it("should create WebSocket URL when authenticated AND has permission", () => {
    ...
    expect(capturedUrl).toContain("/api/v1/ws/connections/health");
  });
