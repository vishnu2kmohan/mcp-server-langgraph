...
LUCIDE_ICONS = {
    "X", "Check", "Copy", "Trash", "Trash2", "Edit", "Pencil", "Settings",
    "Plus", "Minus", "ChevronDown", "ChevronUp", "ChevronLeft", "ChevronRight",
    "ArrowLeft", "ArrowRight", "ArrowUp", "ArrowDown", "Search", "Filter",
    "MoreHorizontal", "MoreVertical", "Menu", "Close", "Refresh", "RefreshCw",
    "RotateCcw", "Download", "Upload", "Share", "Link", "ExternalLink",
    "Eye", "EyeOff", "Lock", "Unlock", "Star", "Heart", "Bell", "BellOff",
    "Play", "Pause", "Stop", "SkipForward", "SkipBack", "Volume", "VolumeX",
    "Maximize", "Maximize2", "Minimize", "Minimize2", "Fullscreen", "ZoomIn",
    "ZoomOut", "Home", "User", "Users", "Mail", "Send", "MessageSquare",
    "FileText", "Folder", "FolderOpen", "Save", "Clock", "Calendar",
    "AlertCircle", "AlertTriangle", "Info", "HelpCircle", "CheckCircle",
    "XCircle", "Loader", "Loader2", "Sparkles", "Wand", "Bot", "Brain",
    "Code", "Terminal", "Database", "Server", "Cloud", "Wifi", "WifiOff",
    "Power", "PowerOff", "Moon", "Sun", "Palette", "Sliders", "Tool",
    "Wrench", "Cog", "Grid", "List", "Table", "BarChart", "BarChart2",
    "LineChart", "PieChart", "TrendingUp", "TrendingDown", "Activity",
    "Zap", "Target", "Crosshair", "Map", "MapPin", "Navigation", "Compass",
    "Globe", "Flag", "Bookmark", "Tag", "Hash", "AtSign", "Paperclip",
    "Image", "Camera", "Video", "Mic", "MicOff", "Headphones", "Speaker",
    "Printer", "Clipboard", "ClipboardCheck", "ClipboardList", "Archive",
    "Package", "Box", "Gift", "ShoppingCart", "CreditCard", "DollarSign",
    "Percent", "Calculator", "Award", "Trophy", "Medal", "Crown", "Shield",
    "Key", "Fingerprint", "Scan", "QrCode", "Barcode", "Layers", "Layout",
    "Sidebar", "PanelLeft", "PanelRight", "Columns", "Rows", "Square",
    "Circle", "Triangle", "Hexagon", "Octagon", "Diamond", "Move", "Grab",
    "Hand", "Pointer", "MousePointer", "Cursor", "Type", "Bold", "Italic",
    "Underline", "Strikethrough", "AlignLeft", "AlignCenter", "AlignRight",
    "AlignJustify", "Indent", "Outdent", "ListOrdered", "ListUnordered",
    "Quote", "Heading1", "Heading2", "Heading3", "Code2", "Braces", "Brackets",
    "ToggleLeft", "ToggleRight", "SwitchCamera", "Repeat", "Shuffle",
    "CircleDot", "Disc", "Record", "Radio", "Tv", "Monitor", "Laptop",
    "Smartphone", "Tablet", "Watch", "Gamepad", "Keyboard", "Mouse",
    "Cpu", "HardDrive", "MemoryStick", "Usb", "Bluetooth", "Cast", "Airplay",
    "Rss", "Chrome", "Github", "Gitlab", "Twitter", "Facebook", "Instagram",
    "Linkedin", "Youtube", "Twitch", "Slack", "Discord", "Figma", "Dribbble",
}
...
def fix_cancel_buttons(content: str) -> tuple[str, list[Fix]]:
    ...
    pattern = re.compile(
        r'(<Button\b)([^>]*?)(\s*>\s*)(Cancel|Close|Back|Dismiss|No|Never mind)(\s*</Button>)',
        re.DOTALL
    )
    ...
    def replacer(match):
        ...
        fixes.append(Fix(line_num, original, f'<Button variant="secondary"...>{text}</Button>', "cancel-button-missing-secondary"))
...
def fix_destructive_buttons(content: str) -> tuple[str, list[Fix]]:
    ...
    pattern = re.compile(
        r'(<Button\b)([^>]*?)(\s*>\s*)(Delete|Remove|Destroy|Discard)(\s*</Button>)',
        re.DOTALL
    )
    ...
    def replacer(match):
        ...
        line_num = find_line_number(content, match.start())
        original = match.group(0).replace("\n", " ")[:60]
        fixes.append(Fix(line_num, original, f'<Button variant="danger"...>{text}</Button>', "destructive-button-missing-danger"))
...
def fix_icon_buttons(content: str) -> tuple[str, list[Fix]]:
    ...
    pattern = re.compile(
        rf'(<Button\b)([^>]*?)(\s*>\s*)(<({icon_names})\b[^/]*/>\s*)(</Button>)',
        re.DOTALL
    )
