...
TEMPLATES: list[ConnectionTemplate] = [
    ...
    ConnectionTemplate(
        id="duckdb",
        name="DuckDB",
        description="In-process analytical database for fast local queries",
        icon="database",
        auth_type="none",
        default_url="http://localhost:3015",
        category="data",
        keywords=["duckdb", "analytics", "olap", "sql", "local", "embedded", "parquet", "csv", "arrow"],
        popularity=65,
        ...
        config_fields=[
            ConfigField(
                ...
                description="URL of the DuckDB MCP server",
            ),
        ],
    ),
    ConnectionTemplate(
        ...
        description="Query and analyze data in Amazon Redshift data warehouse",
        icon="cloud",
        auth_type="api_key",
        default_url="https://your-cluster.region.redshift.amazonaws.com/mcp",
        category="data",
        keywords=["redshift", "aws", "amazon", "data warehouse", "sql", "analytics", "cloud", "query"],
        popularity=70,
        documentation_url="https://docs.aws.amazon.com/redshift/",
        config_fields=[
            ConfigField(
                name="url",
                label="Redshift Cluster URL",
                type="url",
                required=True,
                placeholder="https://your-cluster.region.redshift.amazonaws.com",
                description="Your Amazon Redshift cluster endpoint",
            ),
            ConfigField(
                name="api_key",
                ...
                type="password",
                required=True,
                placeholder="postgresql://user:pass@host:5439/db",
                description="Redshift connection string or IAM auth token",
            ),
        ],
    ),
    ConnectionTemplate(
        ...
        category="data",
        keywords=["trino", "presto", "distributed", "sql", "big data", "analytics", "federated", "query"],
        popularity=55,
        documentation_url="https://trino.io/docs/current/",
        config_fields=[
            ConfigField(
                name="url",
                ...
                description="Trino coordinator HTTP interface URL",
            ),
            ConfigField(
                name="api_key",
                label="Authentication",
                type="password",
                required=False,
                placeholder="user:password or JWT token",
                description="Trino authentication credentials",
            ),
        ],
    ),
]
