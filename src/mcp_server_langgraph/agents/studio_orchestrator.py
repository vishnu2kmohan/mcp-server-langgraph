...
class StudioOrchestrator(BaseOrchestrator[StudioTask, StudioResult]):
    ...
    def __init__(
        ...
        Args:
            ...
            status_broadcaster: Optional broadcaster for real-time status updates to WebSocket clients
            user_id: Optional user ID for filtering WebSocket broadcasts
        """
        # Store broadcaster and user_id before super().__init__ since we need them for callbacks
        self._status_broadcaster = status_broadcaster
        self._user_id = user_id
        self._task_start_times: dict[str, tuple[str, datetime]] = {}  # task_type -> (task_id, start_time)
    ...
    def _create_task_start_callback(self) -> TaskStartCallback:
        ...
        from mcp_server_langgraph.websocket.handlers.orchestrator_status import (
            OrchestratorStatus,
            TaskCategory as WsTaskCategory,
            TaskInfo,
        )

        async def on_task_start(task: StudioTask) -> None:
            ...
            # Store for completion/failure callback
            self._task_start_times[task.task_type] = (task_id, start_time)

            # Map StudioOrchestrator TaskCategory to WebSocket TaskCategory
            ws_category = WsTaskCategory(task.category.value)
    ...
    def _create_task_complete_callback(self) -> TaskCompleteCallback:
        ...
        async def on_task_complete(task: StudioTask, result: StudioResult) -> None:
            if self._status_broadcaster is None:
                return
    ...
    def _create_task_fail_callback(self) -> TaskFailCallback:
        ...
        async def on_task_fail(task: StudioTask, error: str) -> None:
            if self._status_broadcaster is None:
                return
    ...
    async def execute(self, tasks: list[StudioTask]) -> list[StudioResult]:
        """Execute tasks and broadcast IDLE status when complete.

        Overrides base execute() to add IDLE status broadcast after all
        tasks complete, ensuring WebSocket clients know the orchestrator
