# STG Environment - MCP Server LangGraph
# Cost-optimized AWS EKS infrastructure for stg/testing
...
terraform {
  ...
  # Example: terraform init -backend-config=../../backend-configs/aws-stg.s3.tfbackend
  backend "s3" {}
}
...
locals {
  name_prefix  = "stg-mcp-svr" # Limited to 20 chars for VPC module validation
...
module "vpc" {
  ...
  # STG: Single NAT gateway for cost savings
...
module "eks" {
  ...
  # General-purpose node group (smaller for stg)
  ...
  # No compute-optimized nodes in stg
  ...
  application_secrets_arns = [
    "arn:aws:secretsmanager:${var.region}:${data.aws_caller_identity.current.account_id}:secret:mcp-langgraph-stg/*"
  ]
  enable_xray = true
...
module "rds" {
  ...
  # Database configuration (smaller for stg)
  ...
  # STG: Single-AZ for cost savings
  multi_az = false
...
module "redis" {
  ...
  # Cluster configuration (smaller for stg)
