# GCP STG Environment
# Auto-deployed stg environment from main branch
...
terraform {
  ...
  # Example: terraform init -backend-config=../../backend-configs/gcp-stg.gcs.tfbackend
  backend "gcs" {}
...
locals {
  ...
  name_prefix  = "stg-mcp-server-langgraph"
...
module "project_services" {
  ...
  enable_container_scanning_api = false # Vulnerability scanning not enabled in stg
...
module "vpc" {
  ...
  # Medium-sized CIDR blocks for stg
  ...
  # Auto NAT IPs (stg doesn't need static)
...
module "gke" {
  ...
  # Regional cluster for stg (matches prod)
  ...
  # Backup (weekly for stg)
  enable_backup_plan         = true
  backup_schedule_cron       = "0 2 * * 0" # Sunday 2 AM
  backup_retain_days         = 14
  backup_delete_lock_days    = 0 # Allow immediate deletion for stg (Issue #9)
  ...
  # Deletion protection (moderate for stg)
...
module "cloudsql" {
  ...
  # Database
  default_database_name = "mcp_langgraph_stg"
  ...
  # Read replicas (optional for stg)
...
module "memorystore" {
  ...
  # No cross-region replicas for stg
...
module "workload_identity" {
  ...
  service_accounts = {
    ...
    "keycloak-sa" = {
      gcp_sa_name  = "stg-keycloak"
      display_name = "Keycloak STG SA"
      roles = [
    ...
    "openfga-sa" = {
      gcp_sa_name  = "stg-openfga"
      display_name = "OpenFGA STG SA"
      roles = [
        "roles/logging.logWriter",
      ]
      cloudsql_access = true
    ...
    "stg-mcp-server-langgraph" = {
      gcp_sa_name  = "stg-mcp-slg-sa"
      display_name = "MCP Server STG Application SA"
      roles = [
        "roles/logging.logWriter",
...
module "github_actions_wif" {
  ...
  service_accounts = {
    stg = {
      account_id        = "github-actions-stg"
      display_name      = "GitHub Actions - STG Deployment"
      description       = "Service account for GitHub Actions stg deployments to GKE"
      ...
      # Note: artifact_registry_repositories removed - repository mcp-stg doesn't exist
